...
| **2.1**    | correção de vocabulário (PR-N → Checkpoint N; ritual → Gate) + alinhamento à 0023 (separação implementação/revisão/integração)                                                                                                                                                                                                                                               | Checkpoint 2     | **#32**                | 🔄 implementado (`d686d4b`) — Gate pendente  |
| **2.1a**   | **correção arquitetural pós-dogfooding** — substitui a topologia incorreta do Checkpoint 2.1 (que não era `unit` nem `sequential` e contradizia ADR 0020/0024) pela **topologia canônica da 0023** (`unit`/_stacked_/merge único ao fim; #32 não mergeia cedo)                                                                                                               | Checkpoint 2.1   | **#32**                | 🔄 **EM EXECUÇÃO**                           |
| **3**      | **GG-0003 Consistency Projection Check** — estritamente mecânico: `state.yml` é fonte de verdade + lista FIXA de marcadores literais contraditórios (`"Stage 1 ativo"`, `"gate aberto"`, brief `Pendente`…). **Sem parsing semântico.**                                                                                                                                      | Checkpoint 2     | PR próprio _(stacked)_ | pendente                                     |
...
**Ordem de valor:** Checkpoint 1 ✅ → 2 → 2.1 → **2.1a (corrige topologia → 0023)** → (3 consistência + 4A/4B proveniência: barreiras novas **antes** da migração) → 5 + 6 → **7 (o crítico, protegido)** → 9/10 → 11A/B/C → 8/12 (flex).
