...
| **2.2b**   | **correção da fronteira mecânica (achado Codex PR #33)** — garante conectividade ao evento-base (`pull_request`/`push`) e tolerância a provedores externos declarativos, prevenindo falsos produtores sem criar um workflow engine (protegendo o limite do ADR 0020).                                                                                                                                                                                       | Checkpoint 2.2   | Mesmo PR do 2.2        | 🔄 **EM EXECUÇÃO** (Technical Audit)            |
| **3**      | **GG-0003 Consistency Projection Check** — estritamente mecânico: `state.yml` é fonte de verdade + lista FIXA de marcadores literais contraditórios (`"Stage 1 ativo"`, `"gate aberto"`, brief `Pendente`…). **Sem parsing semântico.**                                                                                                                                                                                                                     | Checkpoint 2.2   | PR próprio _(stacked)_ | pendente                                        |
...
**Ordem de valor:** Checkpoint 1 ✅ → 2 ✅ → 2.1 ✅ → 2.1a ✅ → **2.2 + 2.2b (corrige classe `guardrails` órfão e fronteira mecânica; precede o 3)** → (3 consistência + 4A/4B proveniência: barreiras novas **antes** da migração) → 5 + 6 → **7 (o crítico, protegido)** → 9/10 → 11A/B/C → 8/12 (flex).
