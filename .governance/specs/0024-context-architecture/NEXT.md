...
- **Detector de drift do Disclosure de IA (Cenário B da falsificação do Checkpoint 2.4d, 2026-06-02) — DEFERIDO POR DESENHO.** O `governance-pr-check` valida só a **presença** do header `## Disclosure de IA` (`hasSectionHeader`), nunca a **equivalência** do bloco colado com a projeção derivada (`yarn disclosure` é gerador, fora do `validate`/CI). Logo o PR body funciona como **cache persistido sem invalidação**: gera-se o disclosure, colam-se os marcadores `<!-- fatos-derivados:início/fim -->`, surgem novos reviews/findings, e o bloco fica **stale** sem detecção. Categoria distinta de A/C (que produzem projeção errada _agora_): B é **ausência de sincronização** (projeção correta que pode ficar incorreta _depois_). **Menor correção quando priorizado:** `disclosure:check` (ou extensão do `governance-pr-check`, que já tem `prBody`+nó) que extrai o bloco entre os marcadores e o compara com `deriveDisclosure` recém-derivado; falha/avisa em divergência. O ponto de ancoragem (marcadores) **já existe** no template. **Acopla-se à decisão de granularidade do gate** (mesmo denominador de cobertura do Cenário A).
...
- **Drift fonte↔`AGENTS.md` compilado (observado ao vivo, 2026-06-04) — NÃO tratar agora, observação para investigação futura (levantada pelo owner).** O bloco `<AI_GUIDELINES>` do `AGENTS.md` raiz (e os managed blocks dos provider entrypoints) é **artefato derivado** regenerado por `update`/`adopt` (`applyPointers`), mas o `validate` **não verifica sincronismo** fonte↔compilado. Consequência: uma emenda em `.core/rules/**` (ex.: CORE-09) atualiza `rules.json` (via `build:rules`, dentro do `validate`) mas **não** o bloco compilado do `AGENTS.md` — que fica stale **silenciosamente** até o próximo `update`. Demonstrado nesta sessão: a emenda do CORE-09 exigiu rodar `update` à mão para sincronizar (e o `update` é amplo — toca provider entrypoints/config, churn revertido pelo `format`). Categoria: **ausência de detector de sync** (análoga ao Cenário B do disclosure: artefato correto que fica incorreto _depois_). **Menor correção quando priorizado:** um `agents-sync:check` (ou extensão do `repo-validation`) que recompile o bloco em memória e o compare com o `AGENTS.md` versionado, falhando/avisando em divergência — fechando a lacuna sem forçar `update` manual.
...
12. **OBSERVAÇÃO ARQUITETURAL — projeção que consome DADOS do consolidator, mas não seus INVARIANTES (raiz comum dos achados A/C da falsificação do Checkpoint 2.4d, 2026-06-02).** A falsificação do disclosure encontrou dois defeitos reais que caem na mesma forma: o `disclosureRender` tratava o `consolidate()` (`reviewCheck.ts`) como **fonte de dados**, consumindo só parte do que ele computa — e ignorando justamente as saídas que carregam **validade/completude**. Em **C**, `violations` (estado inválido declarado pelo próprio consolidator) era descartado no `main`. Em **A**, o escopo esperado (`node.checkpoints`) não era cruzado com a cobertura de gates antes de afirmar validação. Corrigidos pontualmente (C: `violations` fatal no `main`; A: cobertura conservadora → `partial` em `deriveDisclosure`). **A raiz, porém, é estrutural:** uma projeção deve consumir o consolidator como **fonte de dados _e_ de invariantes**. **Candidato (não agora):** o `consolidate()` retornar explicitamente algo como `{ valid: boolean, complete?: ..., byCheckpoint, violations }`, tornando **difícil** um consumidor futuro ignorar validade/completude por descuido (hoje é fácil: basta desestruturar só `byCheckpoint`). Vale para qualquer projeção futura sobre review-as-artifact, não só o disclosure. Registrar como observação; promover a refatoração só se uma 2ª projeção repetir o padrão (regra 7 — não over-modelar).
