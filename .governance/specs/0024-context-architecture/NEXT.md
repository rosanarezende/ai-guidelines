...
- **Apply do `smoke` no ruleset vivo (Checkpoint 2.2)** — a Camada 0 já aplicou `repo-validation`; tornar `smoke` required exige novo `gh api PUT` no ruleset, **sob autorização explícita do owner após review de payload** (detect-only por design — automação nunca aplica). Até o apply, `ruleset-drift` reporta drift **esperado** (versionado tem `smoke`, vivo ainda não).
...
- **`topology:check` — guard inverso topologia↔realidade git (Checkpoint 2.3a / F1, deferido)** — hoje a topologia em `state.yml` é SSOT mantida à mão, sem nada que a confronte com PRs/branches/commits reais (foi o que deixou nascer o nó-fantasma do 2.3). Análogo à producibilidade do ruleset (2.2), mas para a topologia: todo nó com `github_pr` resolve a PR real; `cursor` aponta ao PR ativo; `checkpoints`↔commits. **É a pré-condição para promover `governance-pr-check` de _advisory_ a _required_** (`[DEC-0024-G07]` / O5). Abrir como checkpoint próprio quando priorizado — **não** construir agora (escopo explicitamente fora do 2.3a, decisão da owner).
