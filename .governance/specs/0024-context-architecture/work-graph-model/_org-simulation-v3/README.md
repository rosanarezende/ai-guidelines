...
| F1   | **a org file-first** — `acme-governance/` como host central (`business/`, `contracts/`, `intake/`, `intents/`, `decisions/`, `incidents/`, `outcomes/`, `repos.yml`, `trust-policy.yml`) + `repos/<repo>/.governance/` como sidecar dos repos adotados + `_templates/` (rascunhos v3, pendentes do gate da P12)                                     | ✅     |
| F2   | **ferramentas** — `_tools/validate.mjs`, `test-adversarial.mjs`, runtime `_lib/` e checks repo-first/backend/app — mecanismos reais da barra do red-team sem depender de projeção estática                                                                                                                                                          | ✅     |
...
node _tools/check-governance-app.mjs # prova app Next/MUI v2: TypeScript strict + snapshot runtime + next build
...
2. `node _tools/validate.mjs` → os erros aparecem nos checks e no app operacional.
