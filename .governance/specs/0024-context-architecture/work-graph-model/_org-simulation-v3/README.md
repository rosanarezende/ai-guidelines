...
| F6   | aplicar os blocos da F5 NA ORDEM (**I/J/K/L aplicados**: schema fail-closed + resolver + authority/envelope + self-attested targets com colapso logado + fila de revisão de contrato/deps cross-intent) → M: derivação/drift + matcher stub → N: vendor/SRI; então outcomes reais → dashboards; destino da `_org-simulation-v2` e gate da P12 | ⬜     |
