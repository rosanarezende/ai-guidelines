...
| F6   | aplicar os blocos da F5 NA ORDEM (I: schema fail-closed + fixtures adversariais → J: resolver de outcome/attester → K: authority registry + envelope → L: fila de revisão de contrato + deps cross-intent → M: derivação/drift + matcher stub → N: vendor/SRI); então outcomes reais → dashboards; destino da `_org-simulation-v2` e gate da P12 | ⬜     |
