...
| F6   | aplicar os blocos da F5 NA ORDEM (**I/J/K aplicados**: schema fail-closed + resolver + authority/envelope; self-attested targets aceitos com colapso logado e badge) → L: fila de revisão de contrato + deps cross-intent → M: derivação/drift + matcher stub → N: vendor/SRI; então outcomes reais → dashboards; destino da `_org-simulation-v2` e gate da P12 | ⬜     |
