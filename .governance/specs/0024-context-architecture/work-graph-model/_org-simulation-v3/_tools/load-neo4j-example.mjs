// load-neo4j-example.mjs — loader executável do read-model Neo4j gerado.
...
//     node _tools/load-neo4j-example.mjs --apply --source-hash <hash>
...
const outRoot = path.join(here, "..", "_examples", "backends");
...
if (dryRun) {
  ...
  console.log(
    `  apply seguro: node _tools/load-neo4j-example.mjs --apply --source-hash ${model.metadata.contentHash}`
  );
  process.exit(0);
}
