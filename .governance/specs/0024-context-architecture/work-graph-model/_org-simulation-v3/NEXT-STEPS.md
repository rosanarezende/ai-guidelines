...
Limite deliberado/pendente: a v3 ainda não migrou a infraestrutura transacional completa da v2 (adapters sqlite/neo4j/mongo como portas write-capable, event-log completo, read-models `db.json` por repo e app de autoria). Ela é a sim ativa de dogfood file-first; Neo4j já tem export, smoke e loader opcional de projeção, mas continua read-model derivado, não SSOT.
...
7. **Portabilidade v2 → v3:** completar a runtime além dos exemplos: comandos com escrita/event-log, primeiro backend transacional real, matcher executável e authoring app, sem reintroduzir a taxonomia antiga nem apagar os resolvers da v3.
