...
- `proposal.create` e `outcome.publish` já têm dry-run/execute com `base-revision`, idempotency, nonce, authority resolvida, escrita file-first e event-log append-only;
...
Limite deliberado/pendente: a v3 ainda não migrou a infraestrutura transacional completa da v2 (adapters sqlite/neo4j/mongo como portas write-capable, event-log completo para todos os comandos, read-models `db.json` por repo e authoring completo de triage/gate/breakdown). Ela é a sim ativa de dogfood file-first; Neo4j já tem export, smoke e loader opcional de projeção, mas continua read-model derivado, não SSOT.
...
1. **Completar authoring por comandos:** expandir o app Next/MUI de `proposal.create`/`outcome.publish` para triage, gate, intent e breakdown, sempre via command pipeline com resolver e event-log.
