...
Limite deliberado/pendente: a v3 ainda não migrou a infraestrutura transacional completa da v2 (adapters sqlite/neo4j/mongo com conexão real, event-log completo, read-models `db.json` por repo e app de autoria). Ela é a sim ativa de dogfood file-first; a portabilidade de backend além do adapter file precisa virar uma próxima fatia explícita, não uma suposição.
...
7. **Portabilidade v2 → v3:** completar a runtime além dos exemplos: comandos com escrita/event-log, adapters sqlite/neo4j/mongo reais, matcher executável e authoring app, sem reintroduzir a taxonomia antiga nem apagar os resolvers da v3.
