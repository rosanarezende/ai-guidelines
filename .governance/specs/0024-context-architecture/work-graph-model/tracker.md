...
>
> **▶ ESTADO ATUAL (2026-07-01 · pra retomar pós-compactação):** modelo **v2 fechado em 9 lentes** (a taxonomia em famílias/dimensões + o envelope L8 + a confiança L9). **4 auditorias adversariais** feitas e incorporadas ([`_audits/`](_audits/)). O **código** ainda está **pré-migração** (P0 em [`features.md`](features.md): `model.ts` tem os 5 kinds antigos, sem dimensões/envelope/política). **SIMULAÇÃO ADVERSARIAL FEITA (rounds 1+2, Codex atacante · [deliberação](deliberation/2026-07-01-adversarial-simulation-red-team-deliberation.md)):** 20 fixtures red-team × régua de 7 eixos. **Veredito:** o modelo **não** está over-engineered, está **SUB-MECANIZADO** (a L9 é declarativa). **Barra:** só "pega" com **resolver+fail-closed+evidência independente** ("controle nomeado" sem mecanismo = cerimônia). Saída: **nada a podar** · **3 leis novas** (fallback/break-glass · independência · dependência verificável — L9) · **rebaixar** só a verdade semântica de capability · **~11 dentes P0** ([`features.md`](features.md)). **PRÓXIMA FASE:** implementar os **mecanismos P0** + materializar o `red-team-corpus/`.
