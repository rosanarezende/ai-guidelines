...
>
> **▶ ESTADO ATUAL (2026-07-05):** o modelo vive em [`model.yml`](model.yml) (**SSOT normativo v3 limpo**). O `model.yml` anterior, que misturava modelo e histórico deliberativo, foi preservado em [`_archive/model-v2-history-2026-07-05/model.yml`](_archive/model-v2-history-2026-07-05/model.yml). O enum antigo `strategy` foi absorvido: `intent.approach = validate-first|direct` + `signal = none|touches-contract|operational-target`; `execution-unit` usa os nomes finais (`delivery-slice`, `graduation`, `release-rollout`, `discovery`). As antigas open questions Q-strategy/Q-collapse/Q-discover-level/Q-names estão fechadas no próprio SSOT. A org simulada v3 é a frente ativa de dogfood físico: 13 repos acme com código MVP, sidecars `.governance`, `context.json`, repo-work lifecycle, repo-contract registries com interface, trust-policy, outcomes/verdicts, app Next/MUI v2 em TypeScript strict, runtime file-first transacional mínimo e fixtures adversariais; o host valida stale/missing/open/replay por resolver fail-closed. O primeiro `verdict.accept` real já foi gravado por comando em `decisions/verdicts.yml` + `events/events.jsonl`; o segundo outcome real (`intent-checkout-stack`) já exercita contrato com `acme-user-context@v4`; e o primeiro outcome operacional sem intent (`fix-checkout-timeout`) já passa por `standalone.complete` antes de publicar `out-fix-checkout-timeout-2027h1`. Integrações externas agora têm catálogo versionado em [`integration-catalog.yml`](integration-catalog.yml): são opcionais e entram como evidence providers/importers/projections/assistant channels, nunca como SSOT paralelo. **Este tracker é EMBASAMENTO** (o porquê e as lentes), não autoridade. O `_map/` é GERADO do `model.yml` (`generate.mjs` · `--check` semântico · coverage manifest). Próxima fase: walkthrough/revisão pós-R5, matcher executável, authoring completo e mecanização de schemas Zod em `governance-demo/packages/contracts`.
...
10. ⬜ **Re-validação independente (Claude Code/Fable 5 + owner)** — re-percorrer as jornadas big/mid/trio/solo/ciclo anual com o SSOT limpo e a v3 física.
11. ⬜ **Selo do PR #45** — reconciliar PR body/tasks/state da spec + Ready/Human Gate quando a owner decidir.
...
- **Simulação ativa:** [`governance-demo/`](governance-demo/) — dogfood físico repo-first + runtime `backend/` + app `frontend/` + exemplos de read-models derivados + smoke/loader file+Neo4j. Layout aplicado: host central em `governance-demo/acme/governance/`; repos adotados em `governance-demo/acme/repos/<repo>/`; standalone repo-local em `governance-demo/acme/repos/<repo>/.governance/works/*.yml`; incidentes centrais em `governance-demo/acme/governance/incidents/`; exemplos em `governance-demo/backend/examples/read-models/{file,neo4j,sqlite,mongo}`.
- **Produto/app ponta-a-ponta:** [`governance-demo/APP-PRODUCT-STATEMENT.md`](governance-demo/APP-PRODUCT-STATEMENT.md) + [`governance-demo/APP-FUNCTIONAL-SPEC.md`](governance-demo/APP-FUNCTIONAL-SPEC.md) + [`governance-demo/APP-ITERATION-MAP.md`](governance-demo/APP-ITERATION-MAP.md). O antigo `app-requirements.md` foi arquivado.
...
2. **▶ Walkthrough da owner:** percorrer a cadeia `objective → target → intent → repo-work done → outcome → actual` e o caminho `incident → standalone.complete → outcome operacional` nos apps.
3. **▶ Revisão adversarial pós-R5:** revisar outcomes de intent, outcome standalone, contrato, transação file-first e badges de self-attestation.
4. **▶ Próximos dentes:** matcher executável v3 e authoring completo no app Next/MUI.
