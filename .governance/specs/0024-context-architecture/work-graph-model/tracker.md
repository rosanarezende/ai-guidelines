# Tracker v2 — o modelo do trabalho como grafo tipado (7 lentes, do amplo ao genérico)

> **SSOT vivo desta frente.** Síntese **refinada** do que aprendemos na v1 ([`tracker-v1.md`](tracker-v1.md)) — sem o histórico de "era isso → virou isso". As 7 lentes vão do **mais amplo** (propósito) ao **mais genérico** (primitivos: dimensões · arestas · camadas físicas).
...
- **Gate:** decisão humana (não deliberação) — `promoted` | `discarded` (inviável × repriorização, pelo rationale).
- **Ativação:** consolida + move; refs por **id estável** (o host resolve em `candidates/`→`intents/`→`archived/`; mover não quebra link; `git mv` preserva histórico).
...
- **`exploration`**: timebox → investiga → `exploration-answer` (+ `fate`: throwaway/promoted/parked). Gate de viabilidade **antes** do compromisso. 🔶 pergunta falsificável + stop-rule.
- **`experiment`** 🔶: `experiment-brief` sela hipótese+métricas → roda atrás de **flag** (exposição/guardrail/duração/decision-rule) → **`experiment-outcome`** (won/lost/inconclusive) + **cleanup**. won → `delivery`.
- **`incident`** 🔶: **declarar** (severidade) → **mitigar** (destrava merge/CI com prazo, blameless) → **resolver** → **postmortem** (garantido por alerta) → gera fix/maintenance/proposal.
...
- **dependência:** `blocked-by` ⟷ `blocks` (espera um trabalho) · `depends-on` (plataforma/versão) · `coordinates-with` (o **contrato** comum, declarado na intent).
- **investigação:** `answers` (a exploration declara a dúvida que responde) · `supported-by` (a decisão se apoia na evidência).
- **fecho:** `closed-by` (→ answer/outcome/postmortem).
- **histórico:** `supersedes` (decisão nova substitui a antiga; append-only).
- **🔶 novas (da taxonomia):** `occurred-during` · `caused-by` · `related-to` (ligam o **`incident`** reativo a works/intents **sem** ser filho de breakdown).
...
1. **Lifecycles próprios** de `maintenance`/`experiment`/`incident` (Lente 5) — detalhar os campos exigidos + o enforcement das dimensões (→ `features.md`).
2. **O breakdown** (Lente 2/3): quais famílias saem do plano (hipótese: delivery sempre · maintenance quando habilitador · aprendizado quando há incerteza; incident nunca) · granularidade (1 explore-point → 1 delivery?) · quem faz (dono × eng).
3. **As arestas novas** do incident (`occurred-during`/`caused-by`) — materializar.
4. **A nova simulação** (mais robusta): re-modelar a sim sobre a taxonomia v2; arquivar a v2 anterior se preciso — **só depois** de validarmos as lentes aqui.
