...
>
> **▶ ESTADO ATUAL (2026-07-01, noite · pra retomar pós-compactação):** o modelo agora VIVE em [`model.yml`](model.yml) (**SSOT v2** — 3 camadas `intent → execution-unit → repo-work` + business-tier com **outcome governado/resolver** + scaling-law + **governance-profiles** + decision-points; provocações **P1–P11 com desfecho**: P10 = validação de fluxo Codex, 14 gaps aplicados · P11 = revisão adversarial Codex, 13 findings aplicados, **F11 decidido** = soft-mandatory p/ mutação de régua/autoridade no perfil full). **Este tracker é EMBASAMENTO** (as 9 lentes; parcialmente pré-3-camadas — reconciliação pendente, item 3 do Caminho abaixo). A sim adversarial (rounds 1+2) segue válida como **barra**: resolver + fail-closed + evidência independente. O **código** segue **pré-migração** (fatia 1–3/5 entregue; `_app` 4/5 e validação 5/5 pendentes; o `model.ts` está na taxonomia de famílias — 2 gerações atrás do modelo). O `_map/` é **GERADO** do model.yml (`generate.mjs` · `--check` semântico · coverage manifest). **PRÓXIMA FASE: o "Caminho para a validação ponta-a-ponta"** (seção abaixo) — começando pelas open-questions (Q-strategy em curso).
...
1. 🚧 **Fechar o desenho (open-questions do model.yml)** — Q-strategy (guardado × derivado) · Q-collapse (regra exata do colapso da execution-unit) · Q-discover-level (discovery cross-repo) · Q-names (nomes finais). Q-collapse + Q-discover-level destravam o F13 (P11). Co-design, **uma por vez**. **EM CURSO (Q-strategy).**
2. ⬜ **Re-validação independente (Codex)** — os blocos D–H da P11 + o F11 mecanizado foram aplicados **sem re-revisão**; re-percorrer as **5 jornadas** (big experiment · mid migration · trio · solo · virada de ciclo anual) no modelo atual.
3. ⬜ **Reconciliar os docs da frente (anti-drift)** — este tracker (lentes parcialmente pré-3-camadas) · [`features.md`](features.md) (o que a P10/P11 **absorveu no modelo** × o que segue a implementar) · cobertura do `_map` (15 seções não projetadas — decidir o que entra; reavaliar o `--check` no `validate` quando estabilizar).
4. 🚧 **Dogfood físico → VIRADA 2026-07-02: org simulada v3** — a owner decidiu antecipar e ampliar: a org-exemplo da P12 vira **arquivos de verdade** em [`_org-simulation-v3/`](_org-simulation-v3/README.md) (negócio · áreas · times · repos · contratos · intents · reativos + templates), com **apps dedicados** (owner: grafo interativo React+Cytoscape · empresas: visões por perfil full/compact/solo + dashboards derivados). Plano por fases no README da v3.
5. 🚧 **Do papel ao mecanismo (a frente longa)** — **começou na v3**: `_tools/validate.mjs` (lints executáveis: refs · sinal×contrato · review derivado · deps · regra de ouro · SoD do target) + `build-graph.mjs`. Resolver completo do outcome quando os primeiros outcomes publicarem (F6 da v3); decidir destino da `_org-simulation-v2`; os ~11 dentes P0 do red-team + policy-artifacts seguem ([`features.md`](features.md)).
6. ⬜ **Selo do PR #45** — reconciliar PR body/tasks/state da spec + Ready/merge (**Human Gate**).
...
1. **▶ Mecanismos P0 (a fazer agora):** dar **dente** (resolver+fail-closed+evidência independente) aos gaps que a sim marcou — A1 (contrato de aceitação do matcher) · B1 (evidência-mínima→exclui/`unknown`) · C2 (revogação/nonce) · D1 (fallback rastreável) · D2 (ACL por edge local) · E1 (invariantes conteúdo×código×contrato) · F2 (telemetria=ref verificável) · F3 (break-glass) · H1 (pre-commit/quarantine) · X (SoD) · O1 (independência do oráculo). Ver [`features.md`](features.md).
2. **Migração P0 do código** (em paralelo): `model.ts` famílias+dimensões+envelope+política (ver `features.md`).
3. **Materializar o `red-team-corpus/`** + a sim robusta sobre a v2: os 20 ataques verbatim + O1 como fixtures reprodutíveis, **cada expected-outcome com autoria independente** (Lei da independência). Arquivar a `_org-simulation-v2` atual se preciso.
4. **Enforcement** de cada dimensão/envelope/política (lint/workflow/dashboard) — o que separa "modelado" de "confiável".
5. **Breakdown** (L2/L3): granularidade (1 explore-point → N works?) · quem faz (dono × eng) · slicing + aprovação do contract-owner.
