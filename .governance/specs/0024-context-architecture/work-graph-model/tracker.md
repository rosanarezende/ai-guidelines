...
> **Síntese viva desta frente — embasamento, não SSOT.** Versão **refinada** do que aprendemos na v1 ([`_archive/governance-demo-docs-2026-07-05/tracker-v1.md`](_archive/governance-demo-docs-2026-07-05/tracker-v1.md)) — sem o histórico de "era isso → virou isso". As 9 lentes vão do **mais amplo** (propósito) ao **mais genérico** (primitivos: dimensões · arestas · camadas físicas · o envelope transacional · as fronteiras de confiança).
>
> **▶ ESTADO ATUAL (2026-07-09):** o modelo vive em [`model.yml`](model.yml) (**SSOT normativo v3 limpo**). O `model.yml` anterior, que misturava modelo e histórico deliberativo, foi preservado em [`_archive/model-v2-history-2026-07-05/model.yml`](_archive/model-v2-history-2026-07-05/model.yml). O enum antigo `strategy` foi absorvido: `intent.approach = validate-first|direct` + `signal = none|touches-contract|operational-target`; `execution-unit` usa os nomes finais (`delivery-slice`, `graduation`, `release-rollout`, `discovery`). As antigas open questions Q-strategy/Q-collapse/Q-discover-level/Q-names estão fechadas no próprio SSOT. A sim v3 que nasceu como `governance-demo` deixou de ser frente ativa deste repo: virou produto Guilda e foi extraída para o repo irmão `git@github.com:rosanarezende/guilda.git`; a evidência histórica fica em [`_archive/guilda-incubation-2026-07/`](_archive/guilda-incubation-2026-07/), a disposição oficial em [`GUILDA-EXTRACTION-DISPOSITION.md`](GUILDA-EXTRACTION-DISPOSITION.md) e a ponte QRD -> checkpoint em [`GUILDA-QRD-PRESERVATION-MATRIX.md`](GUILDA-QRD-PRESERVATION-MATRIX.md). Integrações externas continuam catalogadas em [`integration-catalog.yml`](integration-catalog.yml): são opcionais e entram como evidence providers/importers/projections/assistant channels, nunca como SSOT paralelo. **Este tracker é EMBASAMENTO** (o porquê e as lentes), não autoridade. O `_map/` é GERADO do `model.yml` (`generate.mjs` · `--check` semântico · coverage manifest). Próxima fase neste repo: reconciliar a Spec 0024 e o PR #45 como trabalho de framework, sem declarar readiness do produto Guilda.
...
> Registrado em 2026-07-01; atualizado em 2026-07-05 após a limpeza normativa do SSOT. A barra (da sim adversarial): **resolver + fail-closed + evidência independente, provado num fluxo real**. O modelo ([`model.yml`](model.yml)) está fechado como contrato normativo; o histórico deliberativo fica arquivado. Estas são as frentes que faltam para o selo, na ordem de execução:
...
7. ✅ **File backend transacional mínimo** — lock global por comando, escrita atômica, event-log append-only, marker de recovery e fixtures de concorrência/replay/crash.
8. ✅ **Generalizar outcome com contrato** — `intent-checkout-stack` publica outcome real e cita `acme-user-context@v4`.
9. ✅ **Operacional sem intent planejada** — `standalone.complete` + outcome standalone no bucket operacional.
10. ✅ **Extrair a sim/produto vivo** — Guilda saiu de `governance-demo` para repo irmão; a sim fica arquivada como evidência.
11. ⬜ **Selo do PR #45** — reconciliar PR body/tasks/state da spec + Ready/Human Gate quando a owner decidir, sem tratar Guilda como v1 validada.
...
- **Intake (pré-ativação, v3):** a materialização física provada na incubação está em `_archive/guilda-incubation-2026-07/acme/governance/intake/proposals.yml` + `_archive/guilda-incubation-2026-07/acme/governance/intake/triage/<proposal-id>.yml`. O layout v2 `registers/candidates/<id>/{register,triage,gate}.yml` é histórico e não deve ser recriado na v3.
- **A intent ativada:** `_archive/guilda-incubation-2026-07/acme/governance/intents/<id>.yml` (só o relevante, consolidado).
...
- **O host histórico** (`_archive/guilda-incubation-2026-07/acme/governance`) auto-descobre os repos, deriva as arestas cross-repo (`provides×consumes` → `coordinates-with`) e agrega os `context.json`.
...
- **A app extraída:** a superfície React/Next + Material UI v2 em TypeScript strict foi evidência da incubação e agora pertence ao repo Guilda. Neste repo, a app não é mais workspace ativo.
...
- **v1 (histórico completo, com as idas-e-vindas):** [`_archive/governance-demo-docs-2026-07-05/tracker-v1.md`](_archive/governance-demo-docs-2026-07-05/tracker-v1.md) — 5 lentes, todas as 🟢 decisões originais.
...
- **Incubação extraída:** [`_archive/guilda-incubation-2026-07/`](_archive/guilda-incubation-2026-07/) — dogfood físico repo-first + runtime `backend/` + app `frontend/` + exemplos de read-models derivados + smoke/loader file+Neo4j preservados como evidência histórica. O produto vivo segue no repo Guilda.
- **Matriz de preservação QRD:** [`GUILDA-QRD-PRESERVATION-MATRIX.md`](GUILDA-QRD-PRESERVATION-MATRIX.md) — mapeia as QRDs da incubação Guilda para os objetivos do checkpoint `artifact-taxonomy-and-model-review-contract`, separando produto vivo, evidência histórica e trabalho de framework.
- **Produto/app ponta-a-ponta:** documentos de produto preservados no arquivo histórico (`APP-PRODUCT-STATEMENT.md`, `APP-FUNCTIONAL-SPEC.md`, `APP-ITERATION-MAP.md`) não governam mais implementação neste repo. O antigo `app-requirements.md` foi arquivado.
- **Templates:** templates da incubação ficam em `_archive/guilda-incubation-2026-07/backend/templates/`; o caminho [`_templates/`](_templates/) é ponte histórica e os templates v2 foram arquivados em [`_archive/templates-v2/`](_archive/templates-v2/).
...
1. **▶ Reconciliar PR #45:** PR body, tasks e state precisam refletir que Guilda foi extraída e que readiness do produto não é escopo deste PR.
2. **▶ Fechar a Spec 0024 como framework:** artifact taxonomy, model-review, research-index e gates sem carregar build/teste de Guilda.
3. **▶ Dogfood futuro:** quando Guilda estiver pronta para se governar, registrar integração entre repo Guilda e `ai-guidelines` como novo trabalho governado, não como continuação silenciosa desta sim.
4. **▶ Decisões append-only remanescentes:** converter warnings legítimos em decisões governadas quando a owner aceitar colapso/exceção.
