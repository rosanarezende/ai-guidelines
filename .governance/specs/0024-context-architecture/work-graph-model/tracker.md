...
4. ⬜ **Dogfood físico** — instanciar **UMA jornada completa** (`objective → intent → unit → repo-works → outcome → dashboard`) em **arquivos** na org acme-\* (layout da L7), passando pelos decision-points; a simulação VALIDA os templates (não inventa).
5. ⬜ **Do papel ao mecanismo (a frente longa)** — resolver do outcome · lints do rollup · `unknown-mutation` · profile-declaration · decision-points como **schema/validador executável**; decidir **migrar a sim atual** (fatia 4/5–5/5 pendente) **× nova sim v2**; os ~11 dentes P0 do red-team + policy-artifacts (endereçados no modelo, sem execução — [`features.md`](features.md)).
