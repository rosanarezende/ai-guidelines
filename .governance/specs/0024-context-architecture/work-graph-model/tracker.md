...
**Cross-repo + camada `intent` (modelo maduro — detalhe na Parte 3 e em [`2026-06-26-cross-repo-feature-graph.md`](research/2026-06-26-cross-repo-feature-graph.md)):** as arestas cross-repo (`coordinates-with`/`depends-on`) vivem **nas entregas** e resolvem em **contratos** (`coordinates-with` = compartilhar um contrato · `depends-on` = esperar um build/versão). **Acima** dos trabalhos há a camada **`intent`** (objetivo durável) que **`breaks-into`** N trabalhos e se **retroalimenta** via `answers` (resolução derivada) + `breaks-into`. Intent **multi-repo** → **registry cross-repo** (plano autorado + banco derivado → dashboard).
...
**✅ A LIB ESTÁ PRONTA (a frente do banco fechou o essencial):** domínio puro + portas + adapters File (`266269ff`/`1a3302e8`, read+write, provados por `_lib/check.ts`: grava o `.governance/` zero-API; `deriveGovernance` → RESOLVED; `decides`→`resolves` reconciliado; `prop-001` na governança) + **runner `_lib/build.ts`** (`5437bbe4`, engine: regenera os db.json read-models) + **a view React consome a lib** (`_viewer/src/dashboard` importa `domain/{model,derive}`). **WORKFLOW:** editar `.governance/` → `node _lib/build.ts` (ou `cd <repo> && npm run build`) → `cd _viewer && npm run dashboards`. **O build são 2 FASES** (ver [`2026-06-29-governance-aggregates-published-projections.md`](research/2026-06-29-governance-aggregates-published-projections.md)): cada repo **PUBLICA** seu `context.json` (com o backend DELE) · o host **AGREGA** os `context.json` **SEM abrir banco** → desenvolver a intent/governança **não exige subir os bancos dos repos** (só ler as projeções publicadas; cada repo publica 1x com o seu backend). **O `context.json` é VERSIONADO** (contrato; freshness via pre-commit hook = `_lib/freshness.ts` no lint-staged — anti-drift, mantém o repo FONTE); os caches (`db.json`/`dashboard.html`) vão pro `.governance/.cache/` (gitignored). **Cada repo tem `package.json` próprio** (`db:up`/`db:down`/`seed`/`build`/`dashboard` — roda o seu backend) e há um **README robusto** em [`_org-simulation-v2/README.md`](_org-simulation-v2/README.md) (explica a sim, a arquitetura e como rodar cada projeto/backend). _(✅ faxina FEITA 2026-06-29: o `_banks/` legado foi ARQUIVADO em `_archive/_banks/` — fora da sim.)_

**✅ ESTADO ATUAL (2026-06-29 + matcher/roteamento 2026-06-30) — LER PRIMEIRO (supersede a retomada histórica abaixo):**

**Modelo (decidido + PROVADO na sim):**

- **A intent NÃO delibera** — q/r/d é etapa de **work/exploration**, não de intent. A intent tem `explores: [{id, subject}]` (renomeado de open-questions; ids `eN`) + `contracts`; a `exploration` (ferramenta) tem `explores` (o subject) + `answers: intent#eN` + `verdict` + `fate`. O **GATE deriva do breakdown**: uma work `derives-from` a exploration = ACEITO · nenhuma = REJEITADO (`derives-from` = PROVENIÊNCIA). **Sem `deliberation.yml` na intent.** _(q/r/d: [`2026-06-29-intent-does-not-deliberate-deliberation.md`](deliberation/2026-06-29-intent-does-not-deliberate-deliberation.md).)_
- **Manifesto-por-repo** (`.governance/manifest.yml`: role/owner/domain/provides/consumes/capabilities/architecture) = a camada de CONHECIMENTO; o host **auto-descobre** + **deriva** as arestas (`provides×consumes`→`coordinates-with`). _(q/r/d: [`2026-06-29-manifest-shape-deliberation.md`](deliberation/2026-06-29-manifest-shape-deliberation.md).)_
- **Host agrega PROJEÇÕES PUBLICADAS, não bancos vivos** — `build.ts` em **2 FASES**: cada repo PUBLICA `context.json` (com o SEU backend) · o host AGREGA os `context.json` **SEM abrir banco** → desenvolver a intent **não exige subir os bancos**. `context.json` = contrato **VERSIONADO** (freshness via `_lib/freshness.ts` no pre-commit/lint-staged); caches (`db.json`/`dashboard.html`/sqlite `governance.db`) → `.governance/.cache/` (gitignored). _(q/r/d: [`2026-06-29-published-projection-deliberation.md`](deliberation/2026-06-29-published-projection-deliberation.md).)_
- **Roteamento VERTICAL (a 🔴🔥) — v1+v2 FEITO + MEDIDO (2026-06-30):** a porta `Matcher` (plugável, **async**) sugere ONDE investigar (`capabilities`) e QUEM entrega (`provides`) — **advisory, NÃO toca o gate** (Q4). Espectro **solo→enterprise** provado e medido: léxico-zero-infra → embed local (`nomic`/`bge-m3`) → LLM local (`gemma3:12b`) → **hosted via PLANO** (Claude/OpenAI, sem tokens, agent-delegate) **ou API** (Gemini). Lição-chave: no **fácil** o barato basta; no **difícil** o barato **COLAPSA** e o LLM ganha; **maior ≠ melhor** (a acurácia satura no médio); a **qualidade da capability** é a alavanca. _(deliberação [`2026-06-29-vertical-routing-deliberation.md`](deliberation/2026-06-29-vertical-routing-deliberation.md) [Q1-Q4 decididas; Q5 parqueada] + research [`2026-06-29-capability-matching-and-llm-research.md`](research/2026-06-29-capability-matching-and-llm-research.md) + **[`MATCHER.md`](_org-simulation-v2/MATCHER.md)** = o espectro + como-rodar cada tier + as tabelas medidas.)_
...
**Fila (próximo):** ✅ **FEITO (até 2026-06-30):** os 2 checks do manifesto (anti-typo + freshness do `architecture`, `_lib/manifest-check.ts`, bloqueantes; fecham D2/D3) · parity de `explores` nos 4 backends · **roteamento vertical v1+v2 COMPLETO** (porta `Matcher` async plugável + `LexicalMatcher` + `deriveRouting`/`deriveTagGraph` + adapters `OllamaEmbed`/`OllamaGenerate`/`CliDelegate`/`GeminiApi` + `matcher.yml` no host + build resiliente) · **bench + stress test MEDIDOS** (`_bench/` + `MATCHER.md`): no **fácil** (tags 1-1) o barato basta; no **difícil** (paráfrase, sem tags, PT) o barato **COLAPSA** e o LLM ganha — léxico 2/8 · nomic-EN 1/8 · `bge-m3`-multi 4/8 · `gemma3:12b` local 8/8(489s) · **Gemini API** flash-lite 6/8(6s)/flash 8/8(60s)/pro 8/8(100s) · **plano-sem-token** Claude 8/8(77s)/OpenAI 8/8(56s); agy/Gemini-CLI trava headless (quirk). **RESTA:** 🔵 **rodada do `_viewer`** (fiar o app vivo à lib + **a viz de roteamento/tag-graph mora AQUI** — dados já no `db.json`) → `Neo4jHostRepository` + arestas da Lente 3 como relações Neo4j → **revisitar a D3** (arestas derivadas, era tentativa). **🅿️ Parqueado:** Q5 (gate de evolução: consultas→proposals/insights) · **gerador de capabilities** (skill/CLI que apoia a IA a escrever/atualizar capabilities — a alavanca da qualidade, reforçada pelo stress) · Gemini `pro` = billing (free tier é por-modelo).

**▶ ATUALIZAÇÃO 2026-06-30 (sessão "app + fluxo de iniciativa"):** construído o **app `_app/`** (do zero, file-first sobre a `_lib`; substitui o `_viewer` legado) com o **fluxo completo registro → triagem → investigação → gate → ativação**: nova estrutura física `acme-governance/registers/{candidates,archived}/` + `intents/`; entidades `Register`/`Triage`/`Gate` na lib (+ `promote`/`discard` que **consolida e move** com `git mv`-quando-rastreado/`fs.rename`); telas de **registro** (negócio), **detalhe da candidata**, **triagem** (eng: itens próprios — de negócio OU da eng — + matcher **simulável** léxico/LLM-local/API + contratos a partir dos `provides`) e **gate**. Deliberações: [`2026-06-30-intent-authoring-shape-deliberation.md`](deliberation/2026-06-30-intent-authoring-shape-deliberation.md) (D1–D10). **O fluxo ponta-a-ponta + o breakdown nos 5 tipos (🔶 stress-test, incl. "incidente cabe?") está em [`2026-06-30-initiative-to-works-flow.md`](research/2026-06-30-initiative-to-works-flow.md)** — companion a alimentar junto com este tracker. **PRÓXIMO (decidido com a owner):** benchmark de gaps (prompt p/ Codex) → fechar os 🔶 do breakdown; depois: disparar exploration de verdade no repo · métrica de tempo-bloqueado · viz/grafos.

**📋 MODELO DE PROMPT DE RETOMADA (template — preencher com o ESTADO ATUAL ↑; copiar pra um chat novo no início da sessão):**

> Estrutura que vem funcionando (a owner cola num chat novo; o repo vence transcript/memória):
>
> 1. **Cabeçalho:** Spec 0024 · branch `feat/spec-0024-...` · "modelo VIVO file-first, org anonimizada `acme-*` em `_org-simulation-v2/`, o REPO vence transcript/memória" · regras (commits incrementais · nunca `--no-verify` · push só do reportado · merge/Ready/Human-Gate/advance = autorização separada).
> 2. **ANTES DE AGIR, LEIA (em ordem):** (a) **este tracker INTEIRO** + o bloco **ESTADO ATUAL**; (b) as **deliberações q/r/d** recentes (`research/2026-06-*-*-deliberation.md`); (c) as **researches** de arquitetura; (d) o **README** da sim + o **`MATCHER.md`**; (e) a **LIB** `_lib/` (`domain/{model,derive,routing}.ts` · `ports.ts` · `adapters/{file,sqlite,neo4j,mongo,llm}/` · `backend.ts` · `matcher.ts` · `build.ts` · `freshness.ts` · `manifest-check.ts` · `scaffold.ts`); (f) os `_templates/`; (g) [se for explorar a inspiração] a análise dela em `temp/` (NÃO-versionada).
> 3. **ONDE ESTAMOS:** o modelo decidido+provado, em 4-6 bullets (intent NÃO delibera · manifesto-por-repo · projeções publicadas · padrão de repo+scaffold · 3 explore-points resolvem · **roteamento/matcher medido**).
> 4. **A FILA:** o que falta com contexto suficiente (o item-cabeça em destaque) — ver a "Fila (próximo)" abaixo.
> 5. **CONTEXTO DA INSPIRAÇÃO (⚠️ CONFIDENCIAL — só pro contexto do chat, NÃO vai pra arquivo):** descrever a fonte de inspiração (empresa/framework) — **o nome NÃO entra aqui**, fica só na nota local fora do versionamento; reforçar a **regra dura**: nunca citar empresa/framework/URLs/termos internos em arquivo VERSIONADO — sempre `acme-*` + prior art pública.
> 6. **SUA TAREFA AGORA:** depois de LER, GERAR um output do que entendeu (modelo · sim · fila · inspiração) pra confirmar orientação. **NÃO** implementar/explorar; confirmar e PERGUNTAR por qual item da fila começar.
