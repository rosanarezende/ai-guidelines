...
| 45    | Auth real + cache escopado         | `/api/auth/[...all]`          | Montar Better Auth via Next.js e garantir que TanStack Query nunca vaze dados entre conta/workspace.                           | Zod session scope + query invalidation        | `iterado-parcial`   | `nao-validado-visual`  | `bloqueado`               | Contrato/modelagem/teste fechados; rota Next Better Auth, auth-client React e helper de cache TanStack existem; UX de login/convite ainda nao prova o fluxo completo. |
