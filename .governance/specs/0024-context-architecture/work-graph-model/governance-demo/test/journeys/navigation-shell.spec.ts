...
test.describe("Navegacao global e shell", () => {
  test("APP-35 navegacao global organiza fluxo sem virar console tecnico", async ({
    ...
    await expect(page.getByTestId("global-navigation-primary")).toContainText(/configura/i);
    ...
    await expect(page.getByTestId("global-navigation-primary")).toContainText(/sem host/i);
    await expect(page.getByTestId("global-navigation-primary")).toContainText(/em breve/i);
