...
5. **Shell local: próxima fatia.** Signup/organizações/onboarding-por-organização JÁ EXISTEM sobre `backend/src/domain/adoption-shell.ts` + `server/adoption/` (file-first, event-log, lock). Falta: vincular governance host a organização não-demo, persistir as ESCOLHAS do onboarding (perfil/papéis/fontes) como comandos governados, cadastro/aceite de pessoas e, depois, adapter de auth real (identity-provider) substituindo o cookie local não assinado.
6. **Mock API + e2e: próxima fatia de produto.** Criar `governance-demo/mock-api/` com Hono + lowdb, seeds resetaveis e scripts `dev:mock`/`test:e2e`; conectar o frontend por `GOVERNANCE_DATA_SOURCE=mock-api` para validar UX antes de migrar cada fluxo para backend real.
7. **Backend R0/R1: pronto para Fable implementar.** [`BACKEND-R0-R1-FINDINGS.md`](BACKEND-R0-R1-FINDINGS.md) registra os achados e o recorte R0/R1; [`CLAUDE-FABLE-BACKEND-R0-R1-PROMPT.md`](CLAUDE-FABLE-BACKEND-R0-R1-PROMPT.md) contem o prompt de retomada para implementar mock-api, e2e e backend real do onboarding/adocao inicial.
