...
18. **APP-45 — Better Auth + TanStack Query antes de telas de auth real:** QRD-45 fechou que o app usa Better Auth pela integracao Next.js (`/api/auth/[...all]` + `toNextJsHandler`), nao TanStack Start. Antes de voltar para UI de signup/convite real, fechar schema/modelagem/teste de session scope, query keys e invalidation/clear de TanStack Query em login/logout/accept-invite/workspace-switch. Membership de portal continua separada de authority governada.
