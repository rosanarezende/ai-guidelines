...
18. **APP-45 — Better Auth + TanStack Query antes de telas de auth real:** QRD-45 fechou que o app usa Better Auth pela integracao Next.js (`/api/auth/[...all]` + `toNextJsHandler`), nao TanStack Start. A base Zod/modelagem/teste de session scope, query keys e invalidation/clear de TanStack Query ja existe em `@demo/contracts` + `backend/tests/auth-query-scope.test.ts`. Proxima subfatia: montar a rota/client Better Auth e ligar invalidation real em login/logout/accept-invite/workspace-switch, mantendo membership de portal separada de authority governada.
