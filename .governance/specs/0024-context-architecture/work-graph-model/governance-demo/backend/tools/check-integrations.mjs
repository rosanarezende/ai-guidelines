// check-integrations.mjs — smoke dos integration adapters + API handlers.
...
check(
  ...
  gitOk.status === "ok" && gitOk.evidence[0]?.detail?.lastCommit?.hash,
...
check(
  ...
  ciOk.status === "ok" && ciOk.evidence[0]?.detail?.exitCode === 0,
  JSON.stringify(ciOk.evidence[0]?.detail ?? {})
);
...
check(
  ...
  obsOk.status === "ok" && obsOk.evidence[0]?.detail?.mode === "fixture",
...
check(
  ...
  obsRepo.status === "ok" && obsRepo.evidence[0]?.detail?.metrics?.length === 2
);
...
check(
  ...
  overview.status === 200 && overview.body.nodes.length > 0 && overview.body.sourceRevision
);
...
check(
  ...
  impact.status === 200 &&
    impact.body.consumers.length > 0 &&
    impact.body.outcomesCiting.length > 0,
  JSON.stringify(impact.body ?? {})
);
...
check(
  ...
  deps.status === 200 && deps.body.works.length > 0 && deps.body.repos.length > 0
);
const conflicts = await handleGraphConflicts();
check(
  "API conflicts expõe colapsos de atestação modelados",
  conflicts.status === 200 &&
    conflicts.body.conflicts.some((conflict) => conflict.kind === "attestation-collapse")
);
...
check(
  ...
  stale.status === 422 && stale.body.issues.some((issue) => issue.rule === "command-stale")
);
