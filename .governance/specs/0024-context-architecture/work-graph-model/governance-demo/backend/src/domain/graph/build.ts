// build.ts — projeção derivada da runtime v3; sem escrita em disco.
import type {
  GovernanceIssue,
  GraphEdge,
  GraphNode,
...
function findGlobalRef(value: unknown): { kind: string; id: string } | null {
...
export type BuildGraphInput = {
  ...
  profiles?: unknown;
  repoContexts?: Array<Record<string, unknown> & { repo: string }>;
...
export function buildGraphReadModel({
  ...
  repoWorks = [],
  repoContracts = [],
}: BuildGraphInput): GraphReadModel {
  const nodes: GraphNode[] = [];
  const edges: GraphEdge[] = [];
  const nodeIds = new Set<string>();
  const edgeIds = new Set<string>();
  const N = (id: string, type: string, label: string, data: unknown = {}): void => {
    if (nodeIds.has(id)) return;
    nodeIds.add(id);
    nodes.push({ id, type, label, data });
  };
  const E = (source: string, target: string, type: string): void => {
  ...
  for (const x of org.areas) {
    ...
    for (const parent of ([] as string[]).concat(x["cascades-from"] || []))
      E(parent, x.id, "cascades-to");
  }
  ...
  for (const x of org.verdicts || []) {
    ...
    if (x["decided-by"]) E(x["decided-by"], x.id, "accepts-verdict");
  ...
  for (const x of org.policy?.["break-glass"] || []) {
    ...
    if (x["requested-by"]) E(x["requested-by"], x.id, "requests-break-glass");
    if (x["approved-by"]) E(x["approved-by"], x.id, "approves-break-glass");
