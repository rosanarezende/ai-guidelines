// org-domain.ts — domínio puro da org simulada v3. Sem filesystem; recebe snapshot carregado por uma porta/adaptador.
...
type SchemaSpec = {
  required: string[];
  optional: string[];
  enums?: Record<string, ReadonlyArray<string | boolean>>;
};
...
function checkSchema(
  ...
  for (const [k, allowedVals] of Object.entries(s.enums || {}))
    if (record[k] !== undefined && !(allowedVals as ReadonlyArray<unknown>).includes(record[k]))
      issues.push({
        level: "error",
        rule: "schema-enum",
        node: nodeId,
        msg: `"${k}: ${record[k]}" inválido em ${type} (aceitos: ${allowedVals.join(" · ")})`,
      });
}

function checkAllSchemas(o: OrgSnapshot, issues: GovernanceIssue[]): void {
...
type SecretFinding = { path: string; value: string; hash: string };
...
export function deriveIntent(it: IntentDef, o: OrgSnapshot): DerivedIntent {
  ...
  let observedSignal: DerivedIntent["observedSignal"] = "none";
  ...
  let collapse: DerivedIntent["collapse"] = "collapsed";
...
type IssueSink = {
  err: (rule: string, node: string, msg: string) => void;
  warn: (rule: string, node: string, msg: string) => void;
};

// ═════════ VALIDAÇÃO SEMÂNTICA (refs · SoD · regra de ouro · sinais · deps · review EXATO) ═════════
export function validateOrg(o: OrgSnapshot): GovernanceIssue[] {
  ...
  const repoById: Record<string, RepoDef> = Object.fromEntries(o.repos.map((r) => [r.id, r]));
  const teamById: Record<string, Team> = Object.fromEntries(o.teams.map((t) => [t.id, t]));
  const targetById: Record<string, Target> = Object.fromEntries(o.targets.map((t) => [t.id, t]));
  const metricById: Record<string, MetricDefinition> = Object.fromEntries(
    o.metrics.map((m) => [m.id, m])
  );
  const intentById: Record<string, IntentDef> = Object.fromEntries(o.intents.map((i) => [i.id, i]));
  const proposalById = Object.fromEntries((o.proposals || []).map((p) => [p.id, p]));
  const outcomeById: Record<string, Outcome> = Object.fromEntries(
    (o.outcomes || []).map((out) => [out.id, out])
  );
  const authById: Record<string, Authority> = Object.fromEntries(
    (o.authorities || []).map((a) => [a.id, a])
  );
  const resolveGlobalRef = (ref: unknown, node: string, field: string): void => {
    ...
    if (!registry[kind]) {
      err("refs", node, `${field} "${ref}" usa kind não suportado "${kind}"`);
      return;
    }
    if (!registry[kind].has(id)) err("refs", node, `${field} "${ref}" não resolve`);
  };
  const resolvePolicySubjectRef = (ref: unknown, node: string, field: string): void => {
    const [kind, id, ...rest] = String(ref || "").split(":");
    if (!kind || !id || rest.length) {
      err("refs", node, `${field} "${ref}" deve usar GlobalRef simples kind:id`);
      return;
    }
    const registry: Record<string, Set<string>> = {
      objective: ids.obj,
  ...
  for (const a of o.areas)
    for (const parent of ([] as string[]).concat(a["cascades-from"] || []))
  ...
  const resolveAuth = (id: string | undefined, node: string, field: string): void => {
    if (id && !authById[id])
  ...
  const pd = o.org["profile-declaration"] || ({} as OrgSnapshot["org"]["profile-declaration"]);
  if (pd["approved-by"]) {
  ...
  const roleCanAccessRepo = (authorityId: string, repoId: string): boolean => {
    ...
    if (authority.of && ids.area.has(authority.of) && ownerTeam?.area === authority.of) return true;
    return false;
  };
  ...
  for (const bg of o.policy?.["break-glass"] || []) {
    ...
    const approver = bg["approved-by"] ? authById[bg["approved-by"]] : undefined;
  ...
  const changedByContract = new Map<string, string[]>();
  for (const it of o.intents) {
  ...
  const intentDeps: Record<string, string[]> = Object.fromEntries(
    o.intents.map((it) => [it.id, it["depends-on"] || []])
  );
  const intentState: Record<string, number> = {};
  const visitIntent = (id: string): boolean => {
  ...
  for (const s of o.standalone) {
    ...
    if (s.routing) {
      ...
      const top = suggestions[0] || ({} as (typeof suggestions)[number]);
  ...
  for (const incident of o.incidents || []) {
    ...
    if (["declared", "mitigating"].includes(incident.status || "")) {
      for (const field of ["declared-by", "detected-at", "telemetry"] as const)
        if (!incident[field])
    ...
    if (["resolved", "postmortem-complete"].includes(incident.status || "")) {
      for (const field of ["mttr", "postmortem"] as const)
        if (!incident[field])
          err("incident-lifecycle", incident.id, `status ${incident.status} exige ${field}`);
    }
...
type OutcomeIndexes = {
  ids: IdRegistry;
  targetById: Record<string, Target>;
  metricById: Record<string, MetricDefinition>;
  intentById: Record<string, IntentDef>;
  repoById: Record<string, RepoDef>;
  authById: Record<string, Authority>;
};

type VerdictIndexes = {
  ids: IdRegistry;
  intentById: Record<string, IntentDef>;
  outcomeById: Record<string, Outcome>;
...
function validateVerdicts(
  o: OrgSnapshot,
  ix: VerdictIndexes,
  { issues, err }: { issues: GovernanceIssue[]; err: IssueSink["err"] }
): void {
...
function resolveOutcomes(o: OrgSnapshot, ix: OutcomeIndexes, { err, warn }: IssueSink): void {
  ...
  for (const out of o.outcomes) {
    ...
    const w = out.window || ({} as Outcome["window"]);
    ...
    for (const f of ["actor", "authority", "idempotency-key", "issued-at", "nonce"] as const)
      if (!env[f])
