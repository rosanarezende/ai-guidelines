...
| Portal/control plane Git-backed     | QRD-36/41; `SPIKE-CONTROL-PLANE-PORTAL.md` | APP-38, APP-39, APP-40, APP-41, SEC-13, ARCH-CP                   | Fixture pura do spike: `acct-rosana`, `acct-business`, `ws-mundo-da-mel`                       | `domain` active: projection sem conteudo/segredo; invite->membership sem authority; snapshot/event-log persistidos; GitHub bridge dry-run; S1c cobre SQLite/PostgreSQL/Neo4j; S1d executa Better Auth HTTP real em SQLite | Better Auth esta provado como superficie de endpoints e SQLite real; PostgreSQL live, GitHub App real, email e produto navegavel ainda nao estao fechados | Proxima decisao: S1e com PostgreSQL live por ambiente + convite/aceite; Neo4j segue como read-model, nao store do portal |
...
4. **APP-36 / APP-37** — separar trilhas de onboarding por contexto de entrada antes de alterar UI interna.
5. **APP-07 / APP-16 / SEC-11 / SEC-12** — pessoas, grupos, convites, papeis propostos e authority efetiva na UI. **Feito no Onboarding e Settings para criador; falta aprofundar aceite real por conta convidada.**
