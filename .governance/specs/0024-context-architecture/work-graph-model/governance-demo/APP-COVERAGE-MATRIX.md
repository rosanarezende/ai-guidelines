...
| Schema e contratos runtime          | `ARCHITECTURE`; `TESTING-STRATEGY`     | Sem contrato APP dedicado                                 | n/a                                                                                            | Parcial: `@demo/contracts` ja usa Zod para payloads mutaveis de onboarding e members/roles; demais familias ainda usam combinadores/validacao ad hoc | Zod ainda nao cobre todas as rotas publicas nem gera JSON Schema                                                    | Expandir Zod por familia de API, com teste fail-closed por rota                                   |
...
1. **Zod por familia de API** — expandir `@demo/contracts` antes de novas telas: `governance-host` → `work-sources` → `assistant/integrations`. As familias mutaveis de onboarding (`status/path/profile/workspace-mode/stack`) e members/roles ja sao as primeiras referencias.
...
- Pelo menos uma familia de rotas mutaveis usa Zod na fronteira HTTP. **Feito
  para `/api/local/onboarding/*` e `/api/local/{members,roles}/*` mutaveis.**
- Os combinadores proprios deixam de ser a fonte principal para essa familia.
  **Feito para as mutacoes de onboarding e members/roles hoje existentes.**
- Teste de rota prova payload invalido e mensagem de erro governada. **Feito
  para onboarding profile/stack e para as rotas mutaveis de members/roles
  (`schema-invalid`).**
