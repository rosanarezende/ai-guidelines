...
1. **Zod por familia de API** — expandir `@demo/contracts` antes de novas telas: `members/roles` → `governance-host` → `work-sources` → `assistant/integrations`. A familia mutavel de onboarding (`status/path/profile/workspace-mode/stack`) ja e a primeira referencia.
...
- Pelo menos uma familia de rotas mutaveis (`/api/local/onboarding/*`) usa Zod
  na fronteira HTTP. **Feito para status/path/profile/workspace-mode/stack.**
- Os combinadores proprios deixam de ser a fonte principal para essa familia.
  **Feito para as mutacoes de onboarding hoje existentes.**
- Teste de rota prova payload invalido e mensagem de erro governada. **Feito
  para `/api/local/onboarding/profile` e `/api/local/onboarding/stack`
  (`schema-invalid`).**
