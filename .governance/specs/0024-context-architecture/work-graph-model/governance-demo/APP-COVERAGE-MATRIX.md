...
| Portal/control plane Git-backed     | QRD-36/41; `SPIKE-CONTROL-PLANE-PORTAL.md` | APP-38, APP-39, APP-40, APP-41, SEC-13, ARCH-CP                   | Fixture pura do spike: `acct-rosana`, `acct-business`, `ws-mundo-da-mel`                       | `domain` active: projection sem conteudo/segredo; invite->membership sem authority; snapshot/event-log persistidos; GitHub bridge dry-run; S1c cobre SQLite/PostgreSQL/Neo4j | Better Auth esta provado como superficie de endpoints e drivers de SQLite/PostgreSQL estao presentes; ainda nao ha DB live, GitHub App real, email ou produto navegavel | Proxima decisao: S1d com fluxo HTTP real e Better Auth persistido; Neo4j segue como read-model, nao store do portal |
