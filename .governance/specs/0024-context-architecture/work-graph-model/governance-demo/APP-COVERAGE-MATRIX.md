...
| Auth real + TanStack Query scope    | QRD-45; mapa 45                                                                | APP-45                                                            | `workspace-shared-convites`                                                                    | `fixme` de UI; decisao fecha Better Auth via Next.js, nao TanStack Start; `@demo/contracts` agora tem `PortalQueryScopeSchema`, `SensitiveCacheEventSchema`, builder de query key escopada e teste `auth-query-scope.test.ts`; frontend expõe `scopedWorkspaceQueryKeys` sem trocar telas ainda                                                                                                            | Falta montar `/api/auth/[...all]`, `auth-client` e wiring real de invalidation/clear em login/logout/accept/switch; APP-45 continua fixme ate a UX/rota existir                                                                                   | Proxima fatia: montar auth route/client e invalidation usando o contrato ja tipado, ainda antes de refazer telas                        |
