...
| --------------------------------------- | ----------- |
| Contratos declarados                    | 59          |
| Contratos `active`                      | 24          |
| Contratos `expected-fail`               | 6           |
| Contratos `fixme`                       | 29          |
| Contratos `deny`                        | 14          |
| Seeds declaradas                        | 26          |
...
| Onboarding por contexto de entrada  | mapa 04/07; spec §3.1                                                          | APP-36, APP-37                                                    | `empty-workspace`, `workspace-authority-personas`                                              | `expected-fail` com sentinela de rota; contratos definem `workspace-setup` e `member-join`                                                                                                                                                                                                                                                                                                                                    | UI ainda mostra onboarding quase unico; criador, convidado, retorno e demo precisam de trilhas diferentes                                                                                                                                         | Implementar escolha de trilha antes de mexer novamente nas etapas internas                                                                     |
...
| Pessoas, times e papeis             | mapa 07/16                                                                     | APP-07, APP-16, APP-37; SEC-11, SEC-12                            | `workspace-groups-teams`, `workspace-shared-convites`, `workspace-authority-personas`          | Criador/Settings `active`; member-join `expected-fail`; self-assigned, proposed, authority derivada e rotas Zod cobertos                                                                                                                                                                                                                                                                                                      | Aceite via pessoa convidada ainda precisa de tela/fluxo dedicado; convite no onboarding ainda nao atribui papel automaticamente ao convidado                                                                                                      | Aprofundar aceite real por conta convidada e fluxo "convite -> signup -> membership -> papel"                                                  |
...
| Auth real + TanStack Query scope    | QRD-45; mapa 45                                                                | APP-45                                                            | `workspace-shared-convites`                                                                    | `fixme` de UX; decisao fecha Better Auth via Next.js, nao TanStack Start; `@demo/contracts` tem `PortalQueryScopeSchema`, `SensitiveCacheEventSchema`, builder de query key escopada e teste `auth-query-scope.test.ts`; app monta `/api/auth/[...all]` com `toNextJsHandler`, `auth-client` React e helper TanStack que aplica diretivas de cache; logout e troca/criacao de workspace ja limpam/invalidam queries sensiveis | Falta fluxo visual completo de signup/login Better Auth, aceite de convite por conta convidada e troca de workspace exibindo o escopo de cache; APP-45 continua `fixme` ate a UX provar membership de portal sem authority governada              | Proxima fatia: adaptar signup/organizations para usar Better Auth como identidade real, mantendo o shell local e authority governada separados |
...
| Integracoes hub/status              | mapa 20/40/41/42/43/44; QRD-44                                                 | APP-11, APP-20, INT-01, INT-02, INT-03, CONS-03, SEC-09           | `workspace-with-integration-statuses`, `workspace-github-work-source`                          | `fixme`; status por workspace usa Zod; backend tem adapters mecanizados para assistant, git, CI, code-quality, code-security OSV/deps.dev e observability; repo tem `supply-chain:check`/`advisory` + workflow OSV-Scanner                                                                                                                                                                                                    | Hub dedicado `/integrations` ainda ausente; status contextual e autoridade nao sao operaveis; code-security gera evidencia, mas ainda nao alimenta resolver/workflow humano                                                                       | Comecar read-only: catalogo, valor, risco, quem solicita/aprova, status efetivo; expor OSV/deps.dev como evidencia, nao gate automatico        |
...
4. **APP-45** — Better Auth + TanStack Query: schema/modelagem/testes de session scope, workspace scope e invalidation/clear de cache fechados; proxima subfatia monta a rota/client real antes de telas de auth real.
5. **APP-05 / APP-06 / APP-15 / CONS-01** — perfil, regra de acumulo e Settings coerentes.
6. **APP-36 / APP-37** — separar trilhas de onboarding por contexto de entrada antes de alterar UI interna.
7. **APP-07 / APP-16 / SEC-11 / SEC-12** — pessoas, grupos, convites, papeis propostos e authority efetiva na UI. **Feito no Onboarding e Settings para criador; falta aprofundar aceite real por conta convidada.**
