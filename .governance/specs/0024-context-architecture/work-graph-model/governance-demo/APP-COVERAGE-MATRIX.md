...
| --------------------------------------- | ----------- |
| Contratos declarados                    | 61          |
| Contratos `active`                      | 25          |
| Contratos `expected-fail`               | 7           |
| Contratos `fixme`                       | 29          |
| Contratos `deny`                        | 14          |
| Seeds declaradas                        | 26          |
...
| ----------------------------------- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| Signup e sessao do portal           | `APP-FUNCTIONAL-SPEC` §2.2; mapa 01/03/45                                      | APP-01, APP-03, APP-45                                            | `blank`, `workspace-shared-convites`                                                           | `active` em `ui-e2e` e `real-route`: `/signup` cria Better Auth, `/api/local/auth/bridge` cria principal `portal-*`, workspace cria organizacao de portal, convite Better Auth e aceite por conta convidada funcionam sem authority governada                                                                                                                                                                                           | Login dedicado, reset de senha, providers externos e aceite de termos ainda nao existem; `/api/local/signup` permanece como compatibilidade local/teste                                                                                           | Proxima fatia de auth: login/retorno de usuario e APP-46 cache scope                                                                    |
...
| Pessoas, times e papeis             | mapa 07/16                                                                     | APP-07, APP-16, APP-37, APP-45; SEC-11, SEC-12                    | `workspace-groups-teams`, `workspace-shared-convites`, `workspace-authority-personas`          | Criador/Settings `active`; member-join `expected-fail`; APP-45 prova convite Better Auth + aceite local por pessoa convidada; self-assigned, proposed, authority derivada e rotas Zod cobertos                                                                                                                                                                                                                                          | Ainda falta tela dedicada de pessoa convidada e aceite de papel proposto; convite no onboarding ainda nao atribui papel automaticamente ao convidado                                                                                              | Aprofundar fluxo "convite -> signup -> membership -> papel" com UI member-join                                                          |
...
| Auth real + TanStack Query scope    | QRD-45; mapa 45                                                                | APP-45, APP-46                                                    | `workspace-shared-convites`                                                                    | APP-45 `active`: Better Auth via Next.js, signup/bridge/workspace/convite/accept sem authority governada; `@demo/contracts` tem `PortalQueryScopeSchema`, `SensitiveCacheEventSchema`, builder de query key escopada e teste `auth-query-scope.test.ts`; app monta `/api/auth/[...all]`, `auth-client` React e helper TanStack que aplica diretivas de cache; logout e troca/criacao de workspace ja limpam/invalidam queries sensiveis | APP-46 segue `expected-fail`: falta UI/sentinela exibindo escopo de cache e invalidação em troca de conta/workspace/accept-invite; login dedicado ainda nao existe                                                                                | Proxima fatia: login/retorno de usuario + APP-46 visual de cache scope                                                                  |
...
4. **APP-45** — Better Auth identity bridge. **Feito:** signup, workspace, convite e aceite por pessoa convidada sem authority governada.
5. **APP-46** — escopo/limpeza visual de cache TanStack por conta/workspace/convite. Continua `expected-fail`.
6. **APP-05 / APP-06 / APP-15 / CONS-01** — perfil, regra de acumulo e Settings coerentes.
7. **APP-36 / APP-37** — separar trilhas de onboarding por contexto de entrada antes de alterar UI interna.
