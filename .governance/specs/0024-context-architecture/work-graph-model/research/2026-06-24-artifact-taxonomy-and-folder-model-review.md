...
| D3  | `artifact-kind:check` **brando** (valor inválido falha; não-classificado = advisory), agora também validando `disposition` e schema mínimo de `pre-coding-review`                                                                                                 | implementado                          |
...
| D9  | Dimensão **`disposição`** (`living` / `evidence` / `legacy` / `open`) adicionada no `artifact-taxonomy.yml` — torna explícito que "lixo" (debris de lacuna de processo, ex.: handoff) **não merece modelagem**                                                    | implementado                          |
| D10 | **`gates/` é o blueprint** de tipo governado saudável (ver §3)                                                                                                                                                                                                    | observação âncora                     |
| D11 | **Body de PR é fonte versionada do repo** (não projeção do GitHub): na data deste review, o lar proposto era `pr-bodies/`; em 2026-07-10, `[DEC-0024-G26]` supersede o caminho físico para `pull-requests/pr-N/body.md`, preservando o princípio.                 | supersedido por G26 no caminho físico |
...
| --------------------- | ----------------------------------------------------------------------------------------------------------- | ----------------- | -------------------------------------- |
| Polo governado (SSOT) | `state.yml`, `tasks.md`, `decision-brief.md`, `plan.md`, `reviews/`, `gates/`, `pull-requests/pr-N/body.md` | máxima            | **não** — natureza vem do lar          |
...
**DEC-0024-G24** foi registrada em `decision-brief.md`: formaliza D7–D11 + a dimensão `disposição`, defere a reorg ampla de pastas e deixa O1/O3/O4 explicitamente abertos. **DEC-0024-G26** supersede posteriormente apenas o caminho físico de D11 (`pr-bodies/` → `pull-requests/pr-N/body.md`) para reunir body, assets e continuações por PR. Esta `pre-coding-review` permanece como evidência do raciocínio que precedeu a DEC (lifecycle correto: pre-coding-review → DEC → execução).
