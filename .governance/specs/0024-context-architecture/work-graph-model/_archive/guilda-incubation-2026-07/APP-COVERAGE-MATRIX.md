...
| Desktop local-first                 | QRD-52; `SPIKE-DESKTOP-LOCAL-FIRST.md`                                         | DESKTOP-01, DESKTOP-02, DESKTOP-03                                | Fixture temporaria de repos Git locais em `node:test`                                          | `domain`/backend active: detecta repos Git locais, preserva `git status --porcelain`, cria `.governance-host/host.yml` + `events/events.jsonl`, gera `sourceRevision` e persiste snapshot local em SQLite sem portal, browser ou servidor                                                                                                                                                                  | Ainda nao ha shell Tauri/Rust, dialogo real de pasta, watcher, empacotamento desktop nem bancada visual `/spikes/desktop-local`                                                                                                                   | S2b bancada interna desktop-local; S2c comparacao Tauri/Rust vs Electron com dialogo real de pasta                         |
| Portal/control plane Git-backed     | QRD-36/41/42/43; `SPIKE-CONTROL-PLANE-PORTAL.md`; `PORTAL-DELIVERY-ROADMAP.md` | APP-38, APP-39, APP-40, APP-41, SEC-13, ARCH-CP                   | Fixture pura do spike: `acct-rosana`, `acct-business`, `ws-acme-honey`                         | `domain` active: projection sem conteudo/segredo; invite->membership sem authority; snapshot/event-log persistidos; GitHub bridge dry-run; S1c cobre SQLite/PostgreSQL/Neo4j; S1d/S1e executam Better Auth HTTP real em SQLite; S1f executa o mesmo fluxo em PostgreSQL live e prova workspace visivel para duas contas; QRD-43 adiciona compose oficial para PostgreSQL compartilhado com check governado | Better Auth esta provado como superficie de endpoints e SQLite/PostgreSQL real incluindo convite/aceite; compose resolve dogfood local do store compartilhado, mas ainda falta governance host Git-backed real/sandbox, email e produto navegavel | Complementar ao desktop: S1g Git-backed bridge real/sandbox -> S1h matriz de custo/hospedagem; Neo4j segue como read-model |
...
| Integracoes hub/status              | mapa 20/40/41/42/43/44; QRD-44                                                 | APP-11, APP-20, INT-01, INT-02, INT-03, CONS-03, SEC-09           | `workspace-with-integration-statuses`, `workspace-github-work-source`                          | APP-11/20, INT-01/02/03 e CONS-03 `active`; hub `/integrations`, Settings e sugestões contextuais mostram a mesma integração; backend tem adapters mecanizados para assistant, git, CI, code-quality, code-security OSV/deps.dev e observability; repo tem `supply-chain:check`/`advisory` + workflow OSV-Scanner                                                                                          | Code-security gera evidencia, mas ainda nao alimenta resolver/workflow humano; ativação real de provider cloud continua bloqueada por approval/egress                                                                                             | Detalhe/ativação governada de provider e GitHub work-source real                                                           |
| Settings como espelho do onboarding | mapa 15/16/18/19/20                                                            | APP-15, APP-16, APP-18, APP-19, APP-20, CONS-01, CONS-02, CONS-03 | `workspace-compact-policy`, `workspace-shared-convites`, `workspace-provider-versioned-source` | Perfil, pessoas/papeis, fontes, assistant basico e resumo de integrações `active`; CONS-03 prova coerência entre Settings, hub e sugestão contextual                                                                                                                                                                                                                                                       | Ainda falta tela de detalhe/ativação de provider real e permissões granulares por authority                                                                                                                                                       | Provider detail + ativação governada                                                                                       |
| Planejamento de ciclo               | mapa 22                                                                        | APP-22, APP-33                                                    | `workspace-planning-progressivo`                                                               | APP-22 `active`: `/planning` cria objetivo/metrica/target via rota Zod e comando `local.planning.save`; `/results` mostra o target planejado como `sem actual valido` sem inflar outcome                                                                                                                                                                                                                   | APP-33 segue `fixme`: a cadeia completa planning -> intake -> triage -> gate -> audit ainda nao existe; thesis/opportunity/allocation seguem contexto progressivo futuro                                                                          | Criar gate/ativacao e auditoria da cadeia antes de declarar lifecycle completo                                             |
| Intake e registro de iniciativa     | mapa 23                                                                        | APP-23                                                            | `workspace-planning-progressivo`                                                               | APP-23 `active`: `/intake` registra problema, aposta, dúvida e target opcional via rota Zod/comando `local.intake.register`; item nasce com status `triage` e aparece em `/triage` após reload                                                                                                                                                                                                             | Ainda não tem anexos, classificação avançada, Cup draft nem promoção para gate/intent                                                                                                                                                             | Ligar intake real ao gate/ativação e ao audit trail completo                                                               |
| Triagem/matcher                     | mapa 24                                                                        | APP-24                                                            | `workspace-provider-versioned-source`, `workspace-planning-progressivo`                        | APP-24 `active`: `/triage` cria item a partir de dúvida, mostra fates, roda matcher determinístico advisory, exige confirmação humana e registra decisão em `/audit`; APP-23 prova fila de intake em `/triage`                                                                                                                                                                                             | Matcher ainda não usa provider real; unknowns são determinísticos nesta primeira fatia                                                                                                                                                            | Ligar provider de matcher e decisão de gate após intake                                                                    |
...
| Auditoria                           | mapa 31                                                                        | APP-31, APP-33, SEC-08                                            | `acme-demo`, `workspace-planning-progressivo`, `workspace-provider-versioned-source`           | APP-31/24 `active`: `/audit` mostra decisões, outcomes, revisions e exceções/break-glass derivadas; também mostra decisão local de matcher confirmada com actor, authority, score, unknowns e sourceRevision local                                                                                                                                                                                         | Ainda é read-only; APP-33 segue `fixme` para cadeia completa planning -> intake -> gate -> audit                                                                                                                                                  | Ligar eventos reais do host e links de origem quando Gate for ativado                                                      |
| Console tecnico                     | mapa 32                                                                        | APP-32                                                            | `acme-demo`                                                                                    | APP-32 `active`: `/console` expõe área técnica com sourceRevision e a Home principal não depende de payload/JSON manual                                                                                                                                                                                                                                                                                    | Console continua avançado e não substitui as telas humanas                                                                                                                                                                                        | Manter como diagnóstico; novas ações do console precisam respeitar baseRevision/authority                                  |
