// repo-works.ts — acknowledgements repo-first das peças de intent.
...
const readYaml = (file: string): RepoWorkClaim =>
  parse(readFileSync(file, "utf8")) as RepoWorkClaim;
...
function claimFile(repo: string, intentId: string | undefined, workId: string | undefined): string {
  return path.join(REPOS_DIR, repo, GOVERNANCE_DIR, "works", `${intentId}--${workId}.yml`);
}

export function loadPublishedRepoWorks(): RepoWorkClaim[] {
...
function checkClosedSchema(claim: RepoWorkClaim, node: string, issues: GovernanceIssue[]): void {
  ...
  for (const key of ["kind", "file", "breakdownHash"] as const)
...
function requireFields(
  claim: RepoWorkClaim,
  fields: ReadonlyArray<keyof RepoWorkClaim & string>,
  node: string,
...
function checkLifecycle(claim: RepoWorkClaim, node: string, issues: GovernanceIssue[]): void {
  const err = (rule: string, msg: string): void => {
    issues.push({ level: "error", rule, node, msg });
  };
...
export type ValidateRepoWorksOptions = {
  publishedClaims?: RepoWorkClaim[];
};

export function validateRepoWorks(
  o: OrgSnapshot,
  options: ValidateRepoWorksOptions = {}
): GovernanceIssue[] {
  ...
  const publishedById = new Map<string, RepoWorkClaim>();
  const authorities = new Set((o.authorities || []).map((a) => a.id));

  let published: RepoWorkClaim[] = [];
  try {
    published = options.publishedClaims || loadPublishedRepoWorks();
  } catch (e) {
    err("repo-work-parse", "repo-works", (e as Error).message);
    return issues;
  }
