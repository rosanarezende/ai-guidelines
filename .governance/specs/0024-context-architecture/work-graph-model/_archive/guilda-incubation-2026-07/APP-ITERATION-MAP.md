...
| 04    | Onboarding - entrada               | `/onboarding`                 | Escolher trilha por contexto de entrada: criador configura workspace; convidado entra como participante; demo faz tour seguro. | onboarding status + step + entry context      | `iterado`           | `testado-automatizado` | `testado-automatizado`    | APP-36/37 ativos: UI separa `workspace-setup` de `member-join`; validacao visual humana fina ainda pendente.                                                    |
...
| 11    | Onboarding - integracoes           | `/onboarding`                 | Mostrar integracoes como opcionais, com disponivel/release-1/em-breve.                                                         | integration backlog/list/test parcial         | `iterado`           | `testado-automatizado` | `testado-automatizado`    | APP-11 ativo: onboarding mostra release-1, alternativa manual e deixa claro que integracao potencializa, nao substitui o file-first.                            |
...
| 22    | Planejamento de ciclo              | `/planning`                   | Criar ciclo, objetivo, metrica e target; contexto opcional progressivo.                                                        | local.planning.save + results dashboard       | `iterado`           | `testado-automatizado` | `testado-automatizado`    | APP-22 ativo: cria objetivo/metrica/target e mostra em Resultados como sem actual valido.                                                                       |
| 23    | Registro de iniciativa             | `/intake`                     | Registrar uma aposta/necessidade sem breakdown tecnico inicial.                                                                | local.intake.register + triage queue          | `iterado`           | `testado-automatizado` | `testado-automatizado`    | APP-23 ativo: `/intake` registra problema/aposta/duvida e envia para fila de triagem sem exigir solucao tecnica.                                                |
| 24    | Triagem/matcher                    | `/triage`                     | Transformar duvidas em itens e sugerir repos/fontes sem decidir sozinho.                                                       | local.triage.confirm + audit                  | `iterado`           | `testado-automatizado` | `testado-automatizado`    | APP-24 ativo: matcher deterministico sugere, humano confirma e auditoria registra a decisao.                                                                    |
...
| 32    | Console tecnico                    | `/console`                    | Dar acesso tecnico a grafo/comandos/event-log sem substituir a UX principal.                                                   | console/read-model/runtime                    | `iterado`           | `testado-automatizado` | `testado-automatizado`    | APP-32 ativo: console tecnico renderiza sourceRevision e a Home segue sem payload/JSON manual.                                                                  |
...
| 37    | Cup/CWP - specialist router        | overlay transversal           | Mudar linguagem e checklist por tela: sources, onboarding, results, triage, audit etc.                                         | roteamento deterministico                     | `iterado`           | `testado-automatizado` | `testado-automatizado`    | CUP-01/02 ativos: overlay C0/C1 mostra especialista por rota, contexto permitido e limites sem provider externo.                                                |
| 38    | Cup/CWP - provider assistivo       | overlay transversal           | Usar Ollama/OpenAI-compatible/cloud-approved apenas depois de policy/egress.                                                   | assistant provider + egress                   | `iterado-parcial`   | `testado-automatizado` | `testado-automatizado`    | CUP-04 ativo: provider cloud aparece bloqueado por egress; provider assistivo real segue fora do C0/C1.                                                         |
| 39    | Cup/CWP - draft action             | overlay transversal           | Preparar rascunho/dry-run de comando com confirmacao humana e audit.                                                           | command runtime + baseRevision + audit futuro | `iterado-parcial`   | `testado-automatizado` | `testado-automatizado`    | CUP-03 ativo: Cup prepara rascunho e exige confirmacao humana; execucao governada real fica para runtime de comando completo.                                   |
| 40    | Integracoes - hub dedicado         | `/integrations`               | Listar providers por valor/status/risco com cards de vantagens, limites e permissoes.                                          | integration backlog + status                  | `iterado`           | `testado-automatizado` | `testado-automatizado`    | INT-01 ativo: hub read-only mostra valor, limites, permissao e status sem escrever estado autoritativo.                                                         |
| 41    | Integracoes - detalhe/permissao    | `/integrations/[id]` ou modal | Mostrar dados acessados, permissoes, quem aprova, riscos, teste e como desativar.                                              | integration detail + authority                | `iterado-parcial`   | `testado-automatizado` | `testado-automatizado`    | INT-01/03 ativos em cards; detalhe dedicado por rota/modal ainda nao existe.                                                                                    |
| 42    | Integracoes - sugestao contextual  | varias rotas                  | Mostrar poucas sugestoes no ponto do fluxo em que elevam confianca ou reduzem trabalho manual.                                 | integration catalog por surface               | `iterado`           | `testado-automatizado` | `testado-automatizado`    | INT-02 ativo: suggestions aparecem em Sources, Results, Work e Triage sem bloquear fluxo manual.                                                                |
| 43    | Integracoes - autoridade/egress    | `/integrations`               | Separar solicitar, aprovar/ativar e usar; bloquear cloud sem authority/policy.                                                 | authority + egress decision                   | `iterado-parcial`   | `testado-automatizado` | `testado-automatizado`    | SEC-09 e CUP-04 ativos: cloud/egress nao aparecem como ativos sem aprovacao; ativacao real segue governada.                                                     |
...
| 45    | Auth real + cache escopado         | `/api/auth/[...all]`          | Montar Better Auth via Next.js; adaptar login/workspace/convite; manter TanStack Query escopado por conta/workspace.           | portal identity + Zod session scope           | `iterado`           | `testado-automatizado` | `testado-automatizado`    | APP-45/46 ativos: magic link Better Auth -> bridge local -> workspace -> convite -> login da pessoa convidada -> accept; shell expõe escopo e limpeza de cache. |
...
| APP-06   | 05, 06               | active               |
| APP-07   | 07, 16               | active               |
| APP-36   | 04, 05, 07, 08, 09   | active               |
| APP-37   | 04, 07, 16           | active               |
...
| APP-34   | 08, 17               | active               |
| INT-01   | 20, 40, 41, 43       | active               |
| INT-02   | 42                   | active               |
| INT-03   | 01, 20, 21, 44       | active               |
| APP-45   | 01, 02, 03, 45       | active               |
| APP-46   | 01, 02, 03, 45       | active               |
| APP-47   | 01, 14               | active               |
| CUP-01   | 34, 35, 36, 37       | active               |
| CUP-02   | 36, 40, 43           | active               |
| CUP-03   | 39                   | active               |
| CUP-04   | 35, 38, 43           | active               |
| SEC-01   | 10, 19, 43           | active               |
...
| Login/logout/aceite/troca de workspace nao reaproveitam cache sensivel.          | `/login`, `/organizations`, `TanStack Query`   | `testado`     | APP-46 active  |
| Settings e `/integrations` mostram o mesmo status efetivo do provider.           | `/settings`, `/integrations`                   | `testado`     | INT-01/CONS-03 |
| Sugestao contextual de integracao explica o que funciona sem a ferramenta.       | varias rotas                                   | `testado`     | INT-02 active  |
...
| Onboarding - entrada           | `setup-guide`/`member-guide` | Explicar a trilha correta: setup do workspace para criador; entrada de participante para convidado.                                 | `iterado`     | APP-36/37 ativos protegem a separacao de contexto.                      |
| Onboarding - perfil            | `setup-guide`                | Traduzir perguntas em perfil recomendado e consequencias de enforcement.                                                            | `nao-iterado` |                                                                         |
| Onboarding - pessoas/papeis    | `authority-guide`            | Explicar quem esta configurando, quais papeis a pessoa assume, o que fica aberto e por que papel para outra pessoa fica `proposed`. | `iterado`     | APP-07 cobre criador; APP-37 cobre a experiencia separada de convidado. |
