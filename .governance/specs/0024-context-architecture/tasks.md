...
- [/] **Checkpoint 2.2** — **correção estrutural pós-dogfooding (precede o 3)**: `ruleset:check` — **producibilidade** (PRIMÁRIO, no `validate`) + **paridade** (SECUNDÁRIO, detect-only no `ruleset-drift.yml`) + **ruleset-as-code** (`.github/rulesets/main-governance.json`) + agregador estável `smoke`; fecha a classe do `guardrails` órfão (`[DEC-0024-F04]`). **PR próprio _stacked_** (`feat/spec-0024-ruleset-producibility`). **EM EXECUÇÃO**; `yarn validate` verde. _(dep: Checkpoint 2.1a)_
- [ ] **Checkpoint 3** — GG-0003 Consistency Projection Check (mecânico, lista fixa de marcadores). **PR próprio _stacked_.** _(dep: Checkpoint 2.2)_
