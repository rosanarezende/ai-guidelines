...
- [/] **Checkpoint 2.3a** — **correção derivada do 2.3 (Architectural Review)**: reconcilia topologia↔realidade (B1 — remove nó-fantasma, dobra Checkpoints em #33), registra `[DEC-0024-G07]` (B2), elimina dual-SSOT do `plan.md` (O1), invariantes de `sequence` no schema (O6 — única/contígua/`execution`-only), robustez do `governance-pr-check` sem parser (O2), decisão `governance-pr-check` **advisory** (O5). Mesmo PR (**#33**). **EM EXECUÇÃO**; `yarn validate` verde. _(dep: Checkpoint 2.3)_
- [ ] **Checkpoint 3** — GG-0003 Consistency Projection Check (mecânico, lista fixa de marcadores). **PR próprio _stacked_.** _(dep: Checkpoint 2.3a)_
