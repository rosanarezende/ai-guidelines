...
- [/] **Checkpoint co-enforcement** (nó `co-enforcement`, seq 9 / CO-3) — **autorizado pelo Human Gate do #41** (`gates/c-co-projection.yml`, 2026-06-12): EnforcementBinding (declaração→enforcement por nó; PIT-0008 estrutural) + `knowledge:compile` (reusa o compilador de regras). Escopo: absorve ex-rule-guardrail (Rule/Guardrail → Constraint + binding); herda o resíduo legacy-transitório do monólito cercado no PR #38 (compiler + token-budget via check-budget/budget-report.mjs + rules-loader via features editoriais) — a migração plena desse substrato para o compilador TS/src acontece AQUI; candidato natural ao wiring do recibo de carga (`assertFreshHandoffReceipt`, testado e não conectado no CO-4) em comandos mutantes. Zero LLM no runtime. PR próprio _stacked_ sobre o #41. _(dep: `co-projection` / Human Gate do #41)_
  - **Sub-checkpoints internos (CO-3, PR #42; Gate único ao fim, modo unit):** fronteiras por dependência (modelo+binding → compilador+manifesto → migração legacy → enforcement+recibo); cada um `validate` verde, sem merge isolado.
    - [x] **CO-3.1 — Constraint + EnforcementBinding** (modelo + fonte estruturada): fatia vertical mínima `rule|guardrail → Constraint normalizada → EnforcementBinding (4 campos) → resolução de superfície (`npm-script`/`registry-command`) → resolução de mecanismo → manifesto determinístico em memória`. Fonte `.core/constraints/constraints.yml` (+ overlay opcional `.governance/constraints.yml`); `GovernedRef{space:"surface"}` + aresta `constrains`; `constraints:check` integrado ao `validate`. Dados reais: GG-0001 + 1 rule CORE. **EM EXECUÇÃO.**
