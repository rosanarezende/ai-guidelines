...
- [/] **Checkpoint co-flow-convergence** (nó `co-flow-convergence`, seq 10) — **ativo após Human Gate do `co-enforcement` e planejado por `[DEC-0024-G10]` (2026-06-16), antes de `co-capture`**: convergência do fluxo ponta a ponta da Spec 0024 para impedir que `co-capture` e `co-events` automatizem um fluxo ainda parcialmente modelado. Escopo: inventário real → modelo canônico → confronto modelo × código → correção integral dos gaps → dogfood ponta a ponta → falsificação → Human Gate. Deve produzir uma única máquina de estados para nova sessão → handoff → work → implementação → review → resolution → disposition → avanço de sub-checkpoint → Ready → Human Gate → transição de nó → integração → merge; matriz explícita conceito/SSOT/projeções/consumidores/validações; invariantes work↔decide, readiness terminal, zero findings, findings antigos, registry/help/wizard/scripts e não-retorno do legado pós-CO-3.5; jornadas reais incluindo consumidor novo/existente, branch/CI/receipt stale, retomada e modo offline/degradado. PR próprio _stacked_ sobre `co-enforcement`; não é spec separada nem housekeeping; não implementa CO-5.
  - [/] **CO-10.1 — inventário real + modelo canônico**: mapear fatos vivos, estados, autoridades, comandos, efeitos permitidos, artefatos, validações, próximos estados e ações proibidas.
