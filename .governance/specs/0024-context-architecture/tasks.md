...
- [/] **Checkpoint 2.4c** — **2 bugs de implementação da auditoria final**: (1) fingerprint deixa de ser concatenação textual (`join("\n")`, ambíguo a `\n` interno) e passa a **serialização canônica** (`JSON.stringify([...])`) no per-finding e no envelope; (2) resolução passa a apontar finding **totalmente qualificado** (`<role>#<id>`) com matching **exato** (fim do `endsWith` que colidia cross-role). Migra schema/parser/templates/README/dogfood/testes (+ regressões: quebra-de-linha e `architectural_review#F1` ≠ `technical_audit#F1`). Mesmo PR (**#33**). **EM EXECUÇÃO**; `yarn validate` verde. _(dep: Checkpoint 2.4b)_
  ...
  >
  > **Forward (autoritativo em `state.yml § topology.planned`):** `toolchain-simplification` (npm puro) → `co-projection` (CO-4; **reordenado antes de CO-3 em 2026-06-11** — `plan.md § Decisões revisitadas`) → `co-enforcement` (CO-3) → `co-flow-convergence` (seq 10; **inserido em 2026-06-16 por `[DEC-0024-G10]` antes de `co-capture`**) → `co-capture` (CO-5) → `co-events` (CO-6) → `housekeeping` → `dualroot-collapse` → `knowledge-readiness` → `integration-final`. Esta lista é **ponteiro**, não 2ª fonte: em divergência, `state.yml` vence.
...
- [/] **Checkpoint co-enforcement** (nó `co-enforcement`, seq 9 / CO-3) — **autorizado pelo Human Gate do #41** (`gates/c-co-projection.yml`, 2026-06-12): EnforcementBinding (declaração→enforcement por nó; PIT-0008 estrutural) + `knowledge:compile` (reusa o compilador de regras). Escopo: absorve ex-rule-guardrail (Rule/Guardrail → Constraint + binding); herda o resíduo legacy-transitório do monólito cercado no PR #38 (compiler + token-budget via check-budget/budget-report.mjs + rules-loader via features editoriais) — a migração plena desse substrato para o compilador TS/src acontece AQUI; candidato natural ao wiring do recibo de carga (`assertFreshHandoffReceipt`, testado e não conectado no CO-4) em comandos mutantes. Zero LLM no runtime. PR próprio _stacked_ sobre o #41. _(dep: `co-projection` / Human Gate do #41; próximo nó planejado após o Gate: `co-flow-convergence`, não `co-capture`)_
  - **Sub-checkpoints internos (CO-3, PR #42; Gate único ao fim, modo unit):** fronteiras por dependência (modelo+binding → compilador+manifesto → migração legacy → enforcement+recibo → colapso do runtime CLI); cada um `validate` verde, sem merge isolado.
    ...
    - [/] **CO-3.5 — colapso integral do runtime CLI** `readiness: ready-for-transition` (`[DEC-0024-G09]`, 2026-06-15): runtime legado removido fisicamente; `/cli` inexistente; `package.json#bin → dist/cli/main.js`; scripts públicos executam `dist`; tarball SEM `package/cli`; `CommandRegistry` como única fonte de comandos/help/interação; `LegacyExecuteFn`/`loadLegacyExecute` inexistentes; `providers` removido sem alias e absorvido por `update --providers`; `init`/`adopt`/`update`/`check-budget` executam em TypeScript; pacote instalado validado em consumidor novo e existente; CI remoto multi-OS verde; guard arquitetural do cutover reforçado. **Critério de saída atendido:** `/cli` inexistente; `bin` executa `dist`; wizard vive em `/src`; nenhuma ponte legacy; tarball + consumidores verdes. _(Escopo é o runtime `/cli` — NÃO a migração ampla do Grupo B, que segue fora da spec.)_
