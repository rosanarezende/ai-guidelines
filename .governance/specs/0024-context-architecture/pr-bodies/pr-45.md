...
A entrega mais recente adiciona o começo do app ponta-a-ponta: ele lê a runtime v3 server-side, mostra planejamento/intake/execução/contratos/outcomes/repos/operação e envia comandos governados para dry-run/execute sem editar YAML diretamente.
...
- Comandos mutáveis mecanizados nesta fatia: `proposal.create` e `outcome.publish`, com `base-revision`, authority resolvida, idempotency, nonce, escrita YAML e event-log append-only.
...
- Authoring completo de triage/gate/intent/breakdown no app; a fatia atual prova a arquitetura e dois comandos mínimos reais.
...
- Runtime file-first deixa de ser só leitura: `proposal.create` e `outcome.publish` executam em comando governado, falham fechado por `base-revision` stale e registram event-log.
...
- Limite declarado: o app ainda não implementa authoring completo de triage/gate/breakdown nem backend transacional multi-store.
