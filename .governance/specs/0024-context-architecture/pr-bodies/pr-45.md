...
Esta atualização também sanitiza a documentação da frente work-graph: registra decisões fechadas, corrige status stale da v3, alinha o perfil compact com o SSOT, cria o catálogo versionado de integrações externas e deixa explícito que adapters são opcionais para evidência/importação/projeção/assistência — não SSOT paralelo.
