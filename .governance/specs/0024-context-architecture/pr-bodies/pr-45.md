...
Esta atualização também fecha o primeiro dente transacional file-first e generaliza outcomes: comando governado roda sob lock global, escrita atômica, event-log append-only e marker de recovery; `intent-cta-upgrade` tem verdict real via runtime e `intent-checkout-stack` publica o segundo outcome real citando contrato.
...
- Limite declarado: o app ainda não implementa backend transacional multi-store write-capable nem outcome operacional sem intent; a runtime v3 prova file-first antes de SQLite/Neo4j/Mongo write-capable.
