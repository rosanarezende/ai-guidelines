...
A atualização de adoção multi-org foi reorganizada para a `governance-demo`: o domínio compartilhado vive em `packages/domain`, os contratos de payload em `packages/contracts`, fixtures de teste em `packages/test-fixtures`, e o app Next/MUI em `frontend/`. O adoption shell (`account → workspace → governance-host → work-sources`) deixou de ser estado improvisado de tela e passou a ter contrato tipado compartilhado.

Atualização de control plane: a `governance-demo` agora prova, em spike versionado, que Better Auth consegue sustentar o fluxo portal mínimo sem assumir autoridade governada: signup, sessão, criação de organização, convite, signup do convidado e aceite persistem em SQLite via HTTP real; PostgreSQL fica disponível como runner live explícito por ambiente; GitHub/governance host continuam como planos separados.

Atualização APP-45: Better Auth saiu do spike para a infraestrutura do app Next em `/api/auth/[...all]`, com `auth-client` React e integração de cache TanStack por diretivas tipadas; logout e troca/criação de workspace já limpam ou invalidam queries sensíveis sem transformar membership de portal em authority governada.

Nota de honestidade de produto: a `governance-demo` segue como produto visual em incubação. O PR prova arquitetura, contratos, testes e fatias de app, mas **não** declara readiness do aplicativo como v1 validada pela owner. A decisão vigente é hibrida: portal para contas/convites/memberships e governance host Git-backed no provedor do usuário, começando por GitHub. Antes de novas grandes telas, o plano deve fechar nome, extração para repo irmão e contrato do GitHub governance host.
...
- Sim file-first consolidada em `work-graph-model/governance-demo/acme/`, com repos acme adotados, códigos MVP, contextos publicados, repo-work ack, registry de contratos, outcomes e trust-policy.
- Runtime DDD da demo em `governance-demo/backend`, com adapter file, domínio/validador, command dry-run/execute mínimo e read-model de grafo.
- App operacional em `governance-demo/frontend/` usando React/Next + Material UI.
- Contrato TypeScript compartilhado em `governance-demo/packages/domain`, `governance-demo/packages/contracts` e `governance-demo/packages/test-fixtures`, separando domínio browser-safe, schemas Zod e fixtures de teste.
- Locale versionado e colocalizado em `governance-demo/frontend/app/**/_locales/pt-br.json`; novas telas devem consumir strings de produto no menor dono estável da copy (view/step/section/componente/subdomínio/shell).
...
- Declarar a `governance-demo` como app v1 validado ou produto pronto para release.
...
- App Next/MUI em `governance-demo/frontend/` cobre a navegação ponta-a-ponta do grafo e consome API routes sobre a runtime da demo.
...
- Facades antigos de `frontend/lib` foram removidos; o app consome `@demo/domain`, `@demo/contracts` e o SDK server-side curado. `tools/checks/check-governance-app.ts` compila packages/backend/mock/frontend e falha se contrato, fronteira browser/server ou build Next quebrarem.
...
- `tools/checks/check-governance-app.ts` prova snapshot + `next build` + guards de fronteira; a suíte `test:e2e` cobre jornadas e contratos funcionais.
...
- APP-45 agora tem rota Better Auth real em `/api/auth/[...all]`, SQLite local sob `.local-state` por padrão, `auth-client` oficial, bridge de sessão para o shell local, criação de workspace vinculada à organização do portal, convite/aceite via Better Auth e prova ativa de que membership de portal não concede authority governada. O isolamento de cache cross-session segue como APP-46 `expected-fail`.
- APP-23 foi ativado no produto: `/intake` registra problema, aposta e pergunta por rota Zod `POST /api/local/intake/initiatives`, persiste `status=triage` no workspace local e `/triage` mostra a fila resultante sem assumir solução técnica.
...
npm --workspace @demo/domain run typecheck
npm --workspace @demo/contracts run typecheck
npm --workspace @demo/test-fixtures run typecheck
npm --workspace acme-governance-backend run test:shell
npm --workspace acme-governance-mock-api run test:api
npm --workspace acme-governance-e2e run contracts:check
