...
- APP-45 agora tem rota Better Auth real em `/api/auth/[...all]`, SQLite local sob `.local-state` por padrão, `auth-client` oficial, e helper de cache que aplica `SensitiveCacheEventSchema`/`sensitiveQueryCacheDirective()` no QueryClient. O fluxo visual de login/convite segue como contrato `fixme`.
