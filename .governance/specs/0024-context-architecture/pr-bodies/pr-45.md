...
A entrega mais recente adiciona o começo do app ponta-a-ponta: ele lê a runtime v3 server-side, mostra planejamento/intake/execução/contratos/outcomes/repos/operação e envia comandos governados para dry-run/execute sem editar YAML diretamente. O fluxo mínimo agora cobre intake → gate → ativação → breakdown → outcome.
...
- Comandos mutáveis mecanizados nesta fatia: `proposal.create`, `gate.decide`, `intent.activate`, `breakdown.apply` e `outcome.publish`, com `base-revision`, authority resolvida, idempotency, nonce, escrita YAML e event-log append-only.
...
- Authoring completo de triage/repo-work/contract no app; a fatia atual prova a arquitetura e o fluxo mínimo real até breakdown.
...
- Runtime file-first deixa de ser só leitura: `proposal.create`, `gate.decide`, `intent.activate`, `breakdown.apply` e `outcome.publish` executam em comando governado, falham fechado por `base-revision` stale e registram event-log.
...
- Limite declarado: o app ainda não implementa authoring completo de triage/repo-work/contract nem backend transacional multi-store.
