...
Esta atualização também fecha o primeiro dente transacional file-first: comando governado roda sob lock global, escrita atômica, event-log append-only e marker de recovery; o primeiro `verdict.accept` real foi executado na org canônica por runtime, não por edição manual.
