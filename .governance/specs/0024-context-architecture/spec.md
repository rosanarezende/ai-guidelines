...
> - **Plano executável = `plan.md`.** A sequência de Checkpoints de absorção (backlog = relatório de auditoria do Codex) foi **dobrada no [`plan.md`](./plan.md)** no Checkpoint 2, aposentando a dependência de arquivo local efêmero. Cada Checkpoint é atômico/reversível, fechado por um **Gate** (Technical Audit → Architectural Review → Human). **Topologia (canônica da 0023 — corrigida no Checkpoint 2.1a):** `#32` é o PR de governança/bootstrap + **linha de integração** da spec (**não mergeia cedo** — é a base da stack); do **Checkpoint 3** em diante, cada um abre **PR real próprio _stacked_** sobre a linha da spec (não off-`main`); **merge em `main` = evento único ao fim** (modo `unit`, ADR 0024), com Integration PR terminal de homologação. Detalhe em [`plan.md § Topologia operacional`](./plan.md).
