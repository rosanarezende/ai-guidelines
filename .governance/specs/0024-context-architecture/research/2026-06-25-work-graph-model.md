...
**PRÓXIMO (retomar AQUI):** (1) **`FileRepository`+`FileHostRepository`** (read+**WRITE** o `.governance/` versionado — preencher q/r/d = gravar arquivo, **zero API**; o write que faltava) → (2) runner/consumidores (a lib regenera os read-models + demo de write provando a gravação) → (3) fiar a **view React** ao consumo via porta → (4) **[round da owner] `Neo4jRepository` num repo** (mesma porta, zero mudança em domínio/view = a prova da pluggability). _(workflow: `node _banks/run.ts` + `cd _viewer && npm run dashboards`; o `_banks/` central read-only é o que o Repository substitui — vira o host.)_
