...
**Fila (próximo) — ordem recomendada (impacto analisado: checks endurecem o que o vertical usa; campos disjuntos):** ✅ **FEITO:** (c-i) anti-typo das arestas + (c-ii) freshness do `architecture` (stack vs `package.json`) — ambos BLOQUEANTES em `_lib/manifest-check.ts` no pre-commit; fecham D3/D2 · parity de `explores` nos 4 backends. **RESTA, nesta ordem:** **🔴🔥 grafo VERTICAL de roteamento** (conhecimento × governança; deliberação `2026-06-29-vertical-routing-deliberation.md` — **GATE 1: Q1/Q3/Q4 decididas · Q2 EM ITERAÇÃO** [`capabilities {text,tags}` + adapter LLM **local**, embasado em `2026-06-29-capability-matching-and-llm-research.md`] · **Q5 nova PARQUEADA** [gate de evolução: consultas→proposals/insights]) → `Neo4jHostRepository` + arestas da Lente 3 como relações no Neo4j → fiar o app vivo do `_viewer`.
