...
**✅ A LIB ESTÁ PRONTA (a frente do banco fechou o essencial):** domínio puro + portas + adapters File (`266269ff`/`1a3302e8`, read+write, provados por `_lib/check.ts`: grava o `.governance/` zero-API; `deriveGovernance` → RESOLVED; `decides`→`resolves` reconciliado; `prop-001` na governança) + **runner `_lib/build.ts`** (`5437bbe4`, engine: regenera os db.json read-models) + **a view React consome a lib** (`_viewer/src/dashboard` importa `domain/{model,derive}`). **WORKFLOW:** editar `.governance/` → `node _lib/build.ts` → `cd _viewer && npm run dashboards`. _(⚠️ o `_banks/run.ts` central está SUPERSEDED — não rodar; grava o shape antigo e conflita. Deletar o `_banks/` legado = faxina futura.)_

**▶ RETOMAR A SIMULAÇÃO AQUI — continuar a delivery `form-component_1` em diante:** adicionar as **decisions** na `acme-design-system/.governance/works/delivery/form-component_1/deliberation.yml` (shape do domínio: `resolves: [{question, into}]` · `supported-by` · `supersedes` · `status` · `decided-at`), seguindo o cenário da seção "Plano de simulação" (d1→q1 · d2→q2 · **d3 `resolves [q4,q2]` `supersedes [d2]`**; q3 fica sem decisão = pending) → `node _lib/build.ts` → ver o gate evoluir (q2 reaberta→re-resolvida; stage `deciding`→`executing` quando só q3 sobra) no dashboard. Depois: o work → `done`, a intent progride; e os outros tipos. ✅ **`Neo4jRepository` FEITO + PROVADO** (`75ae8899`): backend de `acme-mfe-support` trocado pra **Neo4j** (Docker `neo4j:5`, bolt 7687; `node _lib/neo4j-check.ts` faz seed file→grafo + lê pela MESMA porta + deriva), domínio/derive/view **intactos** — a pluggability provada de verdade. _Refinamentos:_ arestas da Lente 3 como **relações** do grafo (hoje nós-only), `Neo4jHostRepository`, e um **selector de backend** no `build.ts` (pra o dashboard de um repo vir do Neo4j). _(rodar o Neo4j: `docker run -d --name neo4j-sim -p 7474:7474 -p 7687:7687 -e NEO4J_AUTH=neo4j/simsim123 neo4j:5`; browser http://localhost:7474.)_ _(estrutura DDD: `_lib/{domain/{model,derive}.ts · ports.ts · adapters/file/{io,FileRepository,FileHostRepository}.ts · build.ts · check.ts}`; typecheck `cd _viewer && npx tsc -p ../_lib/tsconfig.json`; smoke `cd _lib && node check.ts`.)_
...
**Contexto (artefatos — atual):** `_templates/` (raiz `intent`/`registry-entry`/`proposal`/`exploration`/`state` + `briefs/`·`closings/`·`deliberation/` com `question.md`/`research.md`/`decision.md`/`deliberation.yml`) · research: **`2026-06-28-per-repo-self-contained-data-and-view.md`** (o SPEC do Repository, §7) · `2026-06-26-cross-repo-feature-graph.md` + benchmarks (`intent-vs-standalone`/`experiment-origins`/`explorations-discovery`) · **`_org-simulation-v2/`**: `acme-governance/intents/login_1` (+ `deliberation.yml` d1/d2) · `acme-design-system/.governance/` (`form-validation_1` exploration + **`form-component_1` delivery `active`** com `questions/`+`research/`+`deliberation.yml`) · `acme-mfe-identity` (`login-mfe_1` draft) · `acme-mfe-support` (`proactive-support_1` + `help-on-demand_1` + `prop-001`) · **`_lib/`** (a lib DDD: `domain/{model,derive}.ts` + `ports.ts` + `tsconfig.json`) · **`_banks/`** (o central read-only provisório: `run.ts`/`db.ts`/`derive-{repo,deliberation,governance}.ts`/`io.ts`/`types.ts` — vira o host) · **`_viewer/`** (React: `src/dashboard/` a view-lib SSR-ável + `render-dashboards.tsx`; o app vivo antigo segue no modelo central, pendente de fiação) · `_archive/org-simulation-v1/` · `2026-06-24-*` (D1–D9 · §5 ligações).
