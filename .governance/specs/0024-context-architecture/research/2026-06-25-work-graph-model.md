...
- **Ordem = o trabalho real (exploration-first, como a v1):** intent (input) → explorations respondem as q's → **decisões** (deliberation = o gate) → **as deliveries nascem SÓ DEPOIS**, como **arquivos**, do que a decisão `spawns` (não pré-quebradas) → na execução, a delivery tem seu **próprio q/r/d** → closings → a intent fecha.
