...
**PRÓXIMO (retomar AQUI):** (1) ✅ **`FileRepository` FEITO** (`266269ff` — read+write provado por `_lib/check.ts`: grava o `.governance/` versionado, **zero API**); falta **`FileHostRepository`** (intents + `proposal`; **+ reconciliar `decides`→`resolves`** na deliberation.yml da intent — hoje o legado usa `decides`, o domínio usa `resolves`) → (2) runner/consumidores (a lib regenera os read-models + o host agrega; aposenta o `_banks/run.ts` central) → (3) fiar a **view React** ao consumo via porta → (4) **[round da owner] `Neo4jRepository` num repo** (mesma porta, zero mudança em domínio/view = a prova da pluggability). _(estrutura DDD: `_lib/{domain/{model,derive}.ts · ports.ts · adapters/file/{io,FileRepository}.ts · check.ts}` + `tsconfig.json`+`package.json`; typecheck: `cd _viewer && npx tsc -p ../_lib/tsconfig.json`.)_
