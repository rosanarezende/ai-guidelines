...
**Sim v2 + app (a frente VIVA, validada ponta-a-ponta):** o modelo roda numa app **Vite/React/TS + json-server**: **autoria** (Iniciativas · perguntas · decisões · **propostas**) → `db.json` → **banco** (`_banks/derive-app.ts` lê o db.json) → **board AO VIVO**. Refinamentos provados na sim: o **GATE** (respondida ≠ resolvida; decisão = nó **APPEND-ONLY** no `deliberation.yml`, reabrir = `supersedes`, `pending` = DERIVADO, não nó); **intent = só INPUT** (o banco deriva; o A+ vive no BANCO, não no arquivo-fonte); **proposal = captura HUMANA a QUALQUER momento** (2 triagens: captura + disposição) → dashboard filtrável por time/ICE/tag. **Estado do app (`_viewer`):** tem um breakdown ANTERIOR commitado (`c194fa49`/`74fa41aa`: works com peso/deps; plano derivado ready/active/blocked/done · caminho crítico · paralelização), mas no **modelo ANTIGO** (`intent-0001` · ids pré-reorg · status `active` · q/r/d no `db.seed`) → **PENDENTE de reconciliação** ao sim-de-arquivo. **ROTA (owner 2026-06-28):** ✅ **file-first feito** (reorg `.governance/` · ids `slug_num` · `login_1` · `results-in` · breakdown cria works **draft**+`assignee` · banco YAML ponta-a-ponta · `created/updated-at` · plano=entregas) → **próximo: (2) reconciliar o app ao file-sim; (3) execução (atribuir+iniciar→`active`) → q/r/d do work ativo → fecho da intent.** Outros tipos (`fix`/`patch`/`incident`) + o `experiment` do proativo = frentes próprias. **🔴🔥 DESTAQUE PARQUEADO (abrir SÓ depois de fechar o fluxo):** **framework × conhecimento do projeto-alvo** (ver Parte 2) — como o sistema sabe **ONDE** rodar a exploration + **COMO** quebrar (qual repo/work) sem a memória humana → a **camada de CONHECIMENTO** (org/repos/capabilities/contratos) × o grafo de governança = casa provável no `KnowledgeGraph` da 0024.
...
**Contexto (artefatos):** `_templates/` (reorganizado: raiz `intent.yml`·`registry-entry.yml`·`proposal.yml`·`exploration.yml`·`state.yml`; pastas `briefs/`·`closings/`·`deliberation/`) · `../assets/` (diagramas, incl. **`lente3-edge-graph.svg`**) · research: `2026-06-26-cross-repo-feature-graph.md` · `2026-06-26-benchmark-intent-vs-standalone-work.md` · `2026-06-26-benchmark-experiment-origins.md` · `2026-06-27-benchmark-explorations-discovery-modality.md` · **`_org-simulation-v2/`** (a frente viva): `acme-governance/intents/login_1` (intent + `deliberation.yml` d1/d2 aceitas) · cada repo de trabalho em **`.governance/`**: `acme-design-system` (`form-validation_1` exploration + `form-component_1` delivery draft) · `acme-mfe-support` (`proactive-support_1` exploration/POC + `help-on-demand_1` delivery draft + `proposal.yml` prop-001) · **`_banks/`** (TS): `derive-app.ts` lê o `db.json` da app → snapshot (cycle); `run.ts` = banco do YAML (roda ponta-a-ponta: projeta todo work + descobre intents) · **`_viewer/`** = app **Vite/React/TS** (rotas: Iniciativas · cadastro · detalhe · **propostas** · board · `json-server`/`db.json` semeado de `db.seed.json`; board deriva AO VIVO) · **`_archive/org-simulation-v1/`** (a sim antiga de 7 repos, arquivada) · `_archive/repo-simulation-v1/` · `2026-06-24-decided-g25-work-flow-model.md` (D1–D9) · `2026-06-24-governed-work-flow-model.md` (§5 ligações).
