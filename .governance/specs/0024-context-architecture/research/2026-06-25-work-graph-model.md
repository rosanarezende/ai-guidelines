...
**Fila (próximo) — ordem recomendada (impacto analisado: checks endurecem o que o vertical usa; campos disjuntos):** ✅ **FEITO:** (c-i) check anti-typo das arestas (BLOQUEANTE — `_lib/manifest-check.ts` no pre-commit; fecha D3 da deliberação do manifesto) · parity de `explores` nos 4 backends. **RESTA, nesta ordem:** (c-ii) freshness do `architecture` (stack vs `package.json`, via arquitetura-lint; fecha D2) → **🔴🔥 grafo VERTICAL de roteamento** (usar `capabilities` pra sugerir ONDE rodar a exploration + o breakdown — conhecimento × governança) → `Neo4jHostRepository` + arestas da Lente 3 como relações no Neo4j → fiar o app vivo do `_viewer`.
