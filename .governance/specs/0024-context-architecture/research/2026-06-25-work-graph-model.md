...
**Fila (próximo) — ordem recomendada (impacto analisado: checks endurecem o que o vertical usa; campos disjuntos):** ✅ **FEITO:** (c-i) anti-typo das arestas + (c-ii) freshness do `architecture` (stack vs `package.json`) — ambos BLOQUEANTES em `_lib/manifest-check.ts` no pre-commit; fecham D3/D2 · parity de `explores` nos 4 backends. · **roteamento vertical v1** (🔴🔥; Q1–Q4 decididas, gate 2; deliberação `2026-06-29-vertical-routing-deliberation.md`): `capabilities {text,tags}` + porta `Matcher`/`LexicalMatcher` + `deriveRouting`/`deriveTagGraph` no `db.json` — **dogfood VERDE** (reproduz e1→DS · e2→support · contratos). · **roteamento v2 LLM LOCAL** (Ollama: embed tier 1 + generativo tier 2 + `matcher.yml` raiz do host + `routing-bench.ts` + build resiliente; **medido na RTX 3060** — léxico/embed 4/4 [0.0s/0.6s], generativo + lento e NÃO + exato [qwen3:4b 3/4] → **embeddings é o workhorse**; `MATCHER.md`). **RESTA, nesta ordem:** a **rodada do `_viewer`** (fiar o app vivo à lib + **a viz de roteamento/tag-graph mora AQUI** — dados já no `db.json`, evita view dupla) → `Neo4jHostRepository` + arestas da Lente 3 como relações no Neo4j. **🅿️ Parqueado:** Q5 (gate de evolução: consultas→proposals/insights) · **gerador de capabilities** (skill/CLI que apoia a IA a escrever/atualizar as capabilities — qualidade da capability ↑ a do match; insight da owner pós-stress: no difícil, capability ruim → só o LLM sobrevive) · **stress test** (`_bench/`: léxico 2/8 · embed nomic[EN] 1/8 · gemma3:12b 8/8 — o barato COLAPSA, LLM ganha; falta **embed multilíngue** [bge-m3] + **hosted** Claude/OpenAI/Gemini × 3 tamanhos).
