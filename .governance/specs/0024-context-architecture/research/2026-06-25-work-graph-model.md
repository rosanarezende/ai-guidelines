...
| `answers`                     | investigação | a exploration DECLARA a `question` que responde (`resolved` = estado derivado no fecho) |
...
| **intent** (`intent.yml`)             | `breaks-into`                                                                   | `answers` (via `answered-by`, A+)  | `breaks-into` = vista agrupada por status; `answered-by`/`status`/`verdict` das open-questions = DERIVADOS (A+) do `answers` da exploration |
...
| **DELIBERAÇÃO** | `question` · `research` · `decision`                         | o **raciocínio** (q→r→d, o ✦ coração da Lente 2)      | os 3 + um **mapa VIVO** (**`deliberation.yml`**, renomeado de decision-brief; lista APPEND-ONLY de decisões-nó: `decides`/`supported-by`/`spawns`/`supersedes`) |
...
**Estado:** ✅ **Lentes 1-3 fechadas; Lente 4 (famílias: trabalho/ferramentas/deliberação) em CONVERGÊNCIA.** Lente 1 (5 tipos de trabalho + `proposal`/`exploration` ferramentas). Lente 2 (5 momentos · coração ✦). **Lente 4 (famílias)** + casa-de-cada-aresta + `deliberation.yml` ancora arestas. **Lente 3 = 10 arestas, cada uma com critério único** (estrutura · proveniência · dependência · investigação · fecho · histórico); renomeios `grounded-by`→`supported-by` · `verdicted-by`→`closed-by` · `promotes-to`→`results-in` · `resolves`→`answers`; `spawned-by`/`from-spike`/`promoted-to` fundidos/derivados; **princípio: o dado fica no NÓ, não na aresta** (grafo: `../assets/lente3-edge-graph.svg`). **Camada `intent`** na governança global por **org/BU = PASTAS** (sem campo `scope`); a intent é **`.yml`** (`objective`/`references`/`open-questions`/`contracts`/`breaks-into`/`details`); o **`breaks-into` = vista agrupada por status** (`done`/`active`/`draft`, as draft com `blocked-by`). **Registries:** `registry-entry.yml` (raiz de `_templates/`) = base canônica; `proposal` e `exploration` têm template próprio. **Amarra intent↔exploration (forma v2):** a exploration declara `answers: intent#qN`; a intent deriva `answered-by`/`status`/`verdict` (A+); destrave derivado (bancos `_banks/`). **Status** = progresso PRÓPRIO (`draft|active|done`); **bloqueado/pausado = DERIVADO**. **Experiment** = decisão deliberada (benchmark), via `proposal`→intent dedicada (recomendado) ou inline.

**Sim v2 + app (a frente VIVA, validada ponta-a-ponta):** o modelo roda numa app **Vite/React/TS + json-server**: **autoria** (Iniciativas · perguntas · decisões · **propostas**) → `db.json` → **banco** (`_banks/derive-app.ts` lê o db.json) → **board AO VIVO**. Refinamentos provados na sim: o **GATE** (respondida ≠ resolvida; decisão = nó **APPEND-ONLY** no `deliberation.yml`, reabrir = `supersedes`, `pending` = DERIVADO, não nó); **intent = só INPUT** (o banco deriva; o A+ vive no BANCO, não no arquivo-fonte); **proposal = captura HUMANA a QUALQUER momento** (2 triagens: captura + disposição) → dashboard filtrável por time/ICE/tag. **Estado do app:** **BREAKDOWN ✅ commitado** (`c194fa49` = `weight` no template · `74fa41aa` = `_viewer`): o dono cria works com **peso** (`weight: S/M/L/XL`) + dependências (`blocked-by`/`coordinates-with`); o **plano** é DERIVADO (fases ready/active/blocked/done · **caminho crítico** ponderado · **o que paraleliza agora**). **ROTA ATUAL (decisão da owner 2026-06-28): SEGUIR fechando o FLUXO COMPLETO na sim antes de abrir a investigação.** Falta simular: a **execução** (works `draft`→`active`→`done`; destrave em cadeia; a paralelização rodando de fato) → **saídas/fecho de contrato** (o work entrega → o contrato vira fulfilled) → **fecho da intent** (todos os works done → a intent fecha) → exercitar os outros tipos (`fix`/`patch`/`incident`; o `experiment` do proativo é frente própria — ver Frentes de fundo). **🔴🔥 DESTAQUE PARQUEADO (abrir SÓ depois de fechar o fluxo):** **framework × conhecimento do projeto-alvo** (ver Parte 2) — como o sistema sabe **ONDE** rodar a exploration + **COMO** quebrar (qual repo/work) sem a memória humana → a **camada de CONHECIMENTO** (org/repos/capabilities/contratos) × o grafo de governança = casa provável no `KnowledgeGraph` da 0024.
...
**Frentes de fundo (trabalhos próprios, não desta rodada):** `incident` dedicada (template + bypass-com-prazo + alerta + postmortem) · conectar `proposal`↔backlog↔history · **adapters** (Jira/Linear/Azure — adoção incremental) · **banco(s): rodada dedicada de system design** (separar intents/works · banco de proposals na governança · grafos comunicantes · identidade cross-repo + dashboards) · o **experiment do proativo** (via `prop-001` → intent dedicada). _(a **execução das deliveries do login** `draft`→`active`→`done` saiu daqui: virou a **rota atual** da sim — ver Estado.)_

**Contexto (artefatos):** `_templates/` (reorganizado: raiz `intent.yml`·`registry-entry.yml`·`proposal.yml`·`exploration.yml`·`state.yml`; pastas `briefs/`·`closings/`·`deliberation/`) · `../assets/` (diagramas, incl. **`lente3-edge-graph.svg`**) · research: `2026-06-26-cross-repo-feature-graph.md` · `2026-06-26-benchmark-intent-vs-standalone-work.md` · `2026-06-26-benchmark-experiment-origins.md` · `2026-06-27-benchmark-explorations-discovery-modality.md` · **`_org-simulation-v2/`** (a frente viva): `acme-governance/intents/intent-0001` (intent enxuta + `deliberation.yml` com d1/d2 aceitas) · `acme-design-system` (exploration-001 simples) + `acme-mfe-support` (exploration-001 densa/POC + `proposal.yml` prop-001) · **`_banks/`** (TS): `derive-app.ts` lê o `db.json` da app → snapshot (cycle); `run.ts` = simulador YAML (pausado) · **`_viewer/`** = app **Vite/React/TS** (rotas: Iniciativas · cadastro · detalhe · **propostas** · board · `json-server`/`db.json` semeado de `db.seed.json`; board deriva AO VIVO) · **`_archive/org-simulation-v1/`** (a sim antiga de 7 repos, arquivada) · `_archive/repo-simulation-v1/` · `2026-06-24-decided-g25-work-flow-model.md` (D1–D9) · `2026-06-24-governed-work-flow-model.md` (§5 ligações).
