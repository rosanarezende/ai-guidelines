...
- **Roteamento VERTICAL (a 🔴🔥) — v1+v2 FEITO + MEDIDO (2026-06-30):** a porta `Matcher` (plugável, **async**) sugere ONDE investigar (`capabilities`) e QUEM entrega (`provides`) — **advisory, NÃO toca o gate** (Q4). Espectro **solo→enterprise** provado e medido: léxico-zero-infra → embed local (`nomic`/`bge-m3`) → LLM local (`gemma3:12b`) → **hosted via PLANO** (Claude/OpenAI, sem tokens, agent-delegate) **ou API** (Gemini). Lição-chave: no **fácil** o barato basta; no **difícil** o barato **COLAPSA** e o LLM ganha; **maior ≠ melhor** (a acurácia satura no médio); a **qualidade da capability** é a alavanca. _(deliberação [`2026-06-29-vertical-routing-deliberation.md`](2026-06-29-vertical-routing-deliberation.md) [Q1-Q4 decididas; Q5 parqueada] + research [`2026-06-29-capability-matching-and-llm-research.md`](2026-06-29-capability-matching-and-llm-research.md) + **[`MATCHER.md`](../_org-simulation-v2/MATCHER.md)** = o espectro + como-rodar cada tier + as tabelas medidas.)_
...
**▶ ATUALIZAÇÃO 2026-06-30 (sessão "app + fluxo de iniciativa"):** construído o **app `_app/`** (do zero, file-first sobre a `_lib`; substitui o `_viewer` legado) com o **fluxo completo registro → triagem → investigação → gate → ativação**: nova estrutura física `acme-governance/registers/{candidates,archived}/` + `intents/`; entidades `Register`/`Triage`/`Gate` na lib (+ `promote`/`discard` que **consolida e move** com `git mv`-quando-rastreado/`fs.rename`); telas de **registro** (negócio), **detalhe da candidata**, **triagem** (eng: itens próprios — de negócio OU da eng — + matcher **simulável** léxico/LLM-local/API + contratos a partir dos `provides`) e **gate**. Deliberações: [`2026-06-30-intent-authoring-shape-deliberation.md`](2026-06-30-intent-authoring-shape-deliberation.md) (D1–D10). **O fluxo ponta-a-ponta + o breakdown nos 5 tipos (🔶 stress-test, incl. "incidente cabe?") está em [`2026-06-30-initiative-to-works-flow.md`](2026-06-30-initiative-to-works-flow.md)** — companion a alimentar junto com este tracker. **PRÓXIMO (decidido com a owner):** benchmark de gaps (prompt p/ Codex) → fechar os 🔶 do breakdown; depois: disparar exploration de verdade no repo · métrica de tempo-bloqueado · viz/grafos.
