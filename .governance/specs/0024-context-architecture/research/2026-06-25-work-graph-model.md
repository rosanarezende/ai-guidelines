...
**PRÓXIMO (retomar AQUI):** (1) ✅ **Adapters File FEITOS** — `FileRepository` (`266269ff`) + `FileHostRepository` (`1a3302e8`): read+write provados por `_lib/check.ts` (grava o `.governance/` versionado, **zero API**; `deriveGovernance(login_1)` → q1/q2 RESOLVED, contratos known, breakdown; **`decides`→`resolves` reconciliado**; `prop-001` movido p/ `acme-governance/proposals.yml`) → (2) **runner/consumidores** (a lib regenera os read-models db.json + o host agrega; **aposenta o `_banks/run.ts` central**) → (3) fiar a **view React** ao consumo via porta (hoje importa `_banks/types`) → (4) **[round da owner] `Neo4jRepository` num repo** (mesma porta, zero mudança em domínio/view = a prova da pluggability). _(estrutura DDD: `_lib/{domain/{model,derive}.ts · ports.ts · adapters/file/{io,FileRepository,FileHostRepository}.ts · check.ts}` + `tsconfig`+`package.json`; typecheck `cd _viewer && npx tsc -p ../_lib/tsconfig.json`; smoke `cd _lib && node check.ts`.)_
