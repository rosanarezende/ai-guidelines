...
- **`runtime-state becoming source-of-truth creep`** (sinal arquitetural sinalizado pelo owner pós-Passo 8). O índice público ficou forte o suficiente para começar a atrair: coordenação, priorização, ordering, freshness, automação, dashboards, ranking, resolução implícita. **Critério de revisita:** qualquer proposta envolvendo "ordenação automática", "spec mais recente/ativa/relevante", "próxima spec sugerida", "spec stale", "auto continue", "sync inteligente" requer DEC própria — esses ficam **vetados por default** nesta fase. Princípio canônico: o índice é fonte de **descoberta declarada**, não fonte de **decisão derivada**. **Formalizado em `[DEC-0023-G05]` (2026-05-24):** `active-specs.yml` é minimal canonical projection, não coordination source-of-truth; `title`/`base_branch` opcionais não-persistidos. **Gatilho de revisita:** use case consumir campo do índice para **tomada de decisão operacional** (não apenas render/display) reabre como DEC própria.
...
- **REPL structured commands (`gate`/`gaps`/`next`/`quit` digitados) sem cobertura** (`src/cli/workflow.ts:359-378`). Modos alternativos de input no REPL (humano digita o comando em vez de selecionar pelo menu numérico). Pre-existentes da feature de REPL (PR1), não impactados pela refatoração de menu (Item 7.5). **Critério de revisita:** se ≥ 1 usuário externo reportar uso destas formas alternativas OU se a próxima spec do runtime estender as structured commands, escrever 4 tests BDD triviais cobrindo cada `name`. Hoje: cobertura indireta via classifyInput tests.

- **`collectLocalContext.ts` success paths de `gh`/`git` 65% cobertos** (`:67-148, 164-167, 177-180, 203-207`). Tests existentes cobrem o caminho graceful-fail (com `repoRoot: "/repo"` falso → execs falham → catch retorna ""). Sucessos reais não são exercitados. **Critério de revisita: NÃO** expandir via mocks de `execFileSync` (frágil e contraditório ao espírito best-effort). Quando a candidata `governance-dashboard-and-visual-artifacts` materializar (backlog `Now`), incluir integration test com subprocess + fs reais (padrão `tests/integration/workflow-dispatch.test.mjs`).

- **10 arquivos de test sem label `[BR-*]`** — todos pre-existentes ao PR #25 (`app/services/RulesCompilation.test.ts`, `RulesProjection.test.ts`, `domain/registry/Integrity.helper.test.ts`, `domain/rules/RuleZone.test.ts`, `domain/templates/LegacyMirrorContract.test.ts`, `TasksEvidenceDrivenEquivalence.test.ts`, `domain/workspace/ReservedDirsContract.test.ts`, `infrastructure/yaml/NodeRecipeStore.integration.test.ts`, `test-utils/Boundaries.test.ts`, `RulesTopologyConsistency.test.ts`). Tests passam, comportamento correto; só falta padronização editorial. **Critério de revisita:** batch pós-merge — `chore(tests): padroniza BR labels em tests pré-existentes`. Não-bloqueador para fechamento da 0023.

- **`governance-pr-check.ts` 62% cobertura** — pre-existente do PR2-lifecycle (D03/D04). Bloco 203-238 (validações de governance chain integrity em CI) não testado isoladamente; é exercitado em CI real via GitHub Action `governance-pr-check.yml`. **Critério de revisita:** se houver falha de governance-pr-check em CI que tests unitários poderiam ter detectado antes, escrever cobertura. Aceito como cobertura indireta-via-CI.

- **`JsonRulesCatalogSource.ts` 0% + `InquirerPrompts.ts` 40% + `NodeClipboard.ts` 71%** — pre-existentes, adapters de I/O. Nível certo de teste é integration (TTY/clipboard/JSON real), não unit. **Critério de revisita:** se a porta `Prompts` ganhar tipo novo (ex.: `confirm`, `checkbox`) OU se houver bug de clipboard/JSON catalog em consumidor externo, expandir.

#### Nota metodológica — trajetória TDD do hardening (2026-05-23 III)
