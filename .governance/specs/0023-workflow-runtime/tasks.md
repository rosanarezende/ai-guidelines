...
- [x] **1.O.1** `init`/`adopt` provisiona a estrutura canônica mínima em `.governance/specs/` (`roadmap/backlog.md`, `roadmap/historico.md`, `research-library/`, `research-index.md`) — implementa `[DEC-0021-A03]` no provisioner; `Isolation.test.ts` `it.skip` → teste real. _(1.O.1a dirs + 1.O.1b `ensureFile`.)_
...
- [/] **1.O.7** Modos de aterrissagem da stack (`[DEC-0023-O03]`, 2026-05-27): `unit` (default) vs `sequential` (override), Integration não é veículo, `landed-via reconciliation`. **(a)** ADR 0024 amendment + nota ADR 0020 + `[DEC-0023-O03]` — **feito**. **(b1)** `StackOps.closePullRequest` + `mergeCommitSha` + `MergeStack.mode` (unit/sequential) + rollback recipe no plan + testes BDD — **feito** (wizard fixado em `sequential` até b2). **(b2)** wizard 🔀 pergunta o modo (unit/sequential) + detecta Integration PR para reconciliação + renderiza plan unit/rollback + executa landed-via — **feito** (+1 teste). **(c)** body do #27 reescrito (deliverables / lista de PRs + landed-via / CI / rollback / imagem; instruções de autoria como comentários HTML) + `integration-pr-boilerplate.md` distribuível stack-agnostic (`.specify/templates/` + `.ai-guidelines/templates/`) — **feito**. **(c-follow-up)** wiring da mensagem curada do veículo a partir da narrativa do #27 — **deferido** (acopla wizard à estrutura do body / exige parser; capability `vehicleCommitMessage` já existe+testada, só a auto-derivação fica para depois). Reflexão do body novo nos #26/#27 feita via REST (`gh api PATCH`) — `gh pr edit` quebra nesta versão do `gh` (2.45.0) por bug de "Projects (classic)". **(b1-fix)** `GhCli.editPullRequestBase/mergePullRequest/closePullRequest` convertidos de `gh pr <x>` para `gh api` REST pelo mesmo motivo — desbloqueia o merge-stack no R8. Dogfood `unit` no merge real (R8).
- [x] **1.O.N** Pipeline verde (`yarn validate` — 656 testes). _(Recontagem após 1.O.6: novos testes de archive + round-trip. Revalidar ao fechar 1.O.7.)_
