...
- [/] **1.O.7** Modos de aterrissagem da stack (`[DEC-0023-O03]`, 2026-05-27): `unit` (default) vs `sequential` (override), Integration nunca veículo, `landed-via reconciliation`. **(a)** ADR 0024 amendment + nota ADR 0020 + `[DEC-0023-O03]` — **feito**. **(b)** `StackOps.closePullRequest` + `MergeStack.mode` + mensagem curada + rollback no plan + testes BDD — **pendente**. **(c)** body do #27 + `integration-pr-boilerplate.md` — **pendente**. Dogfood `unit` no merge real (R8).
