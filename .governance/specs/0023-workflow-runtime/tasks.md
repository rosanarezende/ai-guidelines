...
- [/] **1.O.7** Modos de aterrissagem da stack (`[DEC-0023-O03]`, 2026-05-27): `unit` (default) vs `sequential` (override), Integration não é veículo, `landed-via reconciliation`. **(a)** ADR 0024 amendment + nota ADR 0020 + `[DEC-0023-O03]` — **feito**. **(b1)** `StackOps.closePullRequest` + `mergeCommitSha` + `MergeStack.mode` (unit/sequential) + rollback recipe no plan + testes BDD — **feito** (wizard fixado em `sequential` até b2). **(b2)** wizard 🔀 pergunta o modo (unit/sequential) + detecta Integration PR para reconciliação + renderiza plan unit/rollback + executa landed-via — **feito** (+1 teste). **(c)** body do #27 + `integration-pr-boilerplate.md` + wiring da mensagem curada do veículo a partir da narrativa do #27 — **pendente**. Dogfood `unit` no merge real (R8).
