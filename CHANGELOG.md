...
- **`NodeClipboard` real** (PR5, [DEC-0023-B01]). Detecta `pbcopy` (macOS), `wl-copy` (Wayland Linux) ou `xclip -selection clipboard` (X11 Linux). Fail-graceful: retorna `false` sem throw quando indisponível. `detached:true` + `unref()` para não bloquear o event loop esperando daemons do clipboard. Substitui o `NoopClipboard` que existia antes.
...
- **Imagens institucionais refreshed** (PR5, 1.H.13). `docs/assets/ai-guidelines-flow.png` regenerada (capa hero v3 — kernel correto `.governance/registry.yml`, honestidade entregue vs. direcional via badges "EM BREVE"). Nova `docs/assets/ai-guidelines-dx-flow.png` adicionada como capa DX secundária (referenciada inline na seção "Operação do ciclo" do README) — fluxo de sessão em 4 momentos (wizard → execução → estado publicado → enforcement L2), com handoff direcional visualmente diferenciado. Samples das 2 variantes DX não-escolhidas preservados em `docs/editorial/sample-*.png` como referência editorial para posts/slides futuros.
...
- **ADR 0022 (Proposta)** — Handoff situado em estado precede distribuição pré-carregada de regras. Reposiciona `AGENTS.md`/`.cursorrules`/`CLAUDE.md`/`.github/copilot-instructions.md` como stubs orientativos de invocação; um comando handoff (ainda **não** materializado) geraria contexto situado a partir dos artefatos vivos. Princípio: `bootstrap de sessão IA é handoff situado em estado, não distribuição pré-carregada de regras`. Materialização vinculada à candidata `handoff-as-first-class` no backlog `Now`.
...
- **Wizard ganha opções 4+5: Integration PR + merge-stack** (Bloco L, [DEC-0023-L01]) — wizard passa a ter **8 opções** no total. Tier 2 transactional. As ops 🔗 (Abrir Integration PR) e 🔀 (merge-stack) são **bloqueadas por gates determinísticos** do `review.md` (cf. bullet "Gate determinístico de Integration readiness"). Opção 4 (`🔗 Abrir Integration PR da spec ativa`) auto-detecta spec, lê body de `<spec_dir>/integration-pr.md`, abre PR via `gh pr create` com title canônico (`[🔗] [Integration] [Spec NNNN] ...`). Opção 5 (`🔀 Executar merge atômico da stack`) detecta PRs da spec via title + topo-sort por base→head chain (sem ranking; falha em stacks ambíguos com erro narrativo) e pergunta o **modo de aterrissagem** (`[DEC-0023-O03]`): **`unit`** (default) mergeia o PR terminal de implementação (1 SHA canônico em `main`) e encerra os demais — e o Integration PR — via **`landed-via reconciliation`** (`gh pr close` com anotação `landed-via: #<veículo> @ <SHA>`, não rejeitados); **`sequential`** (override) mergeia cada PR bottom-up `--squash --delete-branch` (mecânica original, retomável via `skipSteps` no `MergeStackError`). O `plan` imprime a receita de **rollback** do modo antes de executar. Integration PR nunca é veículo de aterrissagem (homologação). Wizard renumerado: 5 opções existentes deslocam (4→6, 5→7, 6→8); icons (📍 📡 🔗 🔀 📋 🔍 🎨) categorizam por gênero sem ranking algorítmico. Cf. ADR 0024 amendment + DEC-B06/B07 reafirmados.
...
- **Modelo de 3 boundaries por spec** (Bloco M, [DEC-0023-M01]): `tasks.md` (execução), `review.md` (prontidão de integração — gates determinísticos) e `release-log.md` (registro pós-merge, condicional). `tasks.md` passa a ser execution-only; CI/smoke/public-facing check/coerência dos PR bodies migram para o `review.md`.
- **Gate determinístico de Integration readiness** (Blocos L+M, [DEC-0023-M01]). `CheckIntegrationReadiness` lê os gates `R*` do `review.md` e bloqueia — fail-safe, sem IA — as ops transacionais do wizard: 🔗 (Integration PR) exige `R1`–`R7` `[x]`; 🔀 (merge-stack) exige também `R8` (merge authorization do owner). Contraparte de `CheckExecutionAuthorized` (PR4) para comandos com side-effect.
...
- **Bootstrap de spec ausente.** Não há comando para criar `state.yml` automaticamente em spec existente nem para criar spec nova com a estrutura mínima. Bootstrap entra em release futura.
