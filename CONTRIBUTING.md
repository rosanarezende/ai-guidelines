...
Este repositório é **AI-governed**: humanos e agentes de IA seguem o mesmo
fluxo de governança. Se você está usando uma IA para contribuir, oriente-a
a ler [`AGENTS.md`](AGENTS.md) antes de qualquer ação.
...
Se for abrir issue, use os templates em
[`.github/ISSUE_TEMPLATE/`](.github/ISSUE_TEMPLATE/) — eles te ajudam a
estruturar o relato de forma útil.
...
2. Faça o ajuste
3. Abra PR em **modo Draft** usando o template
...
Quando: mudança que toca múltiplos arquivos fora de uma feature isolada,
ou que precisa sobreviver a troca de IA/sessão para ser concluída.

```
backlog.md (candidata) → spec em .specify/specs/<slug>/ → branch → commits → PR Draft → CI → Ready → review
```

1. **Registre a intenção** — abra issue com label apropriado, ou proponha
   entrada em
   [`.specify/specs/roadmap/backlog.md`](.specify/specs/roadmap/backlog.md).
2. **Crie a fundação documental** em `.specify/specs/<slug>/` a partir dos
   templates SDD em
...
3. Crie **branch dedicada**: `feat/spec-XXXX-<slug>` (número sequencial
   alocado quando a candidata sai do backlog).
4. **Commits atômicos** por unidade lógica (não agrupe docs + código +
   config num commit só).
...
8. Antes de pedir review, confirme a checklist técnica:
   spec/plan/tasks atualizados, decisões arquiteturais registradas em ADR
   quando necessário, `yarn format` + `yarn check` verdes, ausência de
   contexto pessoal/operacional vazado, e impacto downstream documentado.
9. Solicite review de pelo menos **1 owner** (ver
   [CODEOWNERS](.github/CODEOWNERS)).

### 3. 🧩 Spec consolidada (absorve candidatas relacionadas)

Quando: várias candidatas no backlog tocam o mesmo contrato e fazem mais
sentido como uma spec única que como specs separadas.

**Critério canônico** (Spec 0008):

> "Se a entrega de uma altera o contrato da outra, fundir em uma spec única.
> Caso contrário, manter separadas."

1. Documentar a fusão na própria `spec.md` (seção "Decisão de Fusão").
2. Atualizar candidatas absorvidas em
   [`backlog.md`](.specify/specs/roadmap/backlog.md) com cross-ref à spec
   consolidada.
3. Seguir workflow 2 a partir daí.

### 4. 🤖 Agente IA com autonomia

Quando: você está executando trabalho via Claude Code, Gemini CLI, Codex,
Cursor, Antigravity ou similar.

1. Ler [`AGENTS.md`](AGENTS.md) — Phase 0 obrigatório (Environment Check
   e persistência).
2. Ler [`.core/rules/global-rules.md`](.core/rules/global-rules.md) seção
   **"Workflow com IA"** — regras universais aplicáveis ao agente.
3. Seguir **PR description colaborativo (3 etapas)**:
   - Listar tópicos relevantes para validação humana **antes** do texto
     final;
   - Só escrever o texto após o humano editar/aprovar a lista;
   - Submeter o texto final para um último check humano antes de criar/
     editar o PR.
4. Aprovação humana explícita obrigatória antes de
   `git push`.
...
| Princípios de engenharia (regras) | `.core/rules/global-rules.md`     | AGENTS, CONTRIBUTING      |
...
- [`AGENTS.md`](AGENTS.md) — fluxo obrigatório (humanos e agentes).
- [`.core/rules/global-rules.md`](.core/rules/global-rules.md) —
  princípios de engenharia + workflow com IA.
- [`docs/process/spec-foundation.md`](docs/process/spec-foundation.md) —
  quando abrir spec, como estruturar `spec.md`/`plan.md`/`tasks.md` e como
  fechar débitos/research.
- [`.specify/specs/roadmap/backlog.md`](.specify/specs/roadmap/backlog.md)
  — backlog e candidatas.
