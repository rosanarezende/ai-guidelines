...
export async function copyDirIfChanged(sourceDir, targetDir, dryRun, actions, options = {}) {
  ...
  for (const entry of entries) {
    ...
    // Em Node 20.x, entry.path existe. Em versões mais antigas ou certas variações,
    // usamos uma abordagem defensiva para obter o caminho completo.
    const entryDir = entry.path || entry.parentPath;
    if (typeof entryDir !== "string") {
      // Fallback para quando o motor não fornece o path (caso raro em Node moderno)
      // Aqui assumimos que se não temos o path, não conseguimos reconstruir a árvore
      // mas no Node 20+ readdir(..., {recursive: true}) garante um dos dois.
      continue;
    }
...
export async function ensureExecutable(filePath, dryRun, actions) {
  ...
  // Tentar git chmod para garantir consistência no repositório
  return new Promise((resolve) => {
    ...
    git.on("error", () => resolve()); // Falha silenciosa se git não estiver no PATH
  });
}
