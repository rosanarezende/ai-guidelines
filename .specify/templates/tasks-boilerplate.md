...
> **Boilerplate genérico — variante de referência.** Ao instanciar uma spec real, escolha a variante alinhada ao `Tipo de spec` declarado no header da `spec.md` (cf. `docs/process/spec-foundation.md` § "Tipos de spec"):
>
...
- [ ] **0.1** **Bootstrap**: ler `roadmap/backlog.md` (spec ativa, prioridades, candidatas absorvidas) e `docs/process/spec-foundation.md` § "Tipos de spec".
...
- [ ] **0.9** `NEXT.md` criado **somente se** a spec gerar débitos conscientes (Insights / Itens descartados / Débitos adiados). Sem débitos → não criar.
...
> **Nota — promoção de regra.** Se a spec promover regra editorial / de processo / de IA, classificar explicitamente como **universal** × **opt-in** (cf. `docs/process/spec-foundation.md` § "Categorias de regras").
...
- [ ] **1.A.[COMMIT]** Commit incremental atômico do sub-bloco A (`<tipo>(spec-NNNN): <resumo do sub-bloco>`).
...
- [ ] **3.5** PR criado/atualizado: descrição em 3 etapas (contexto → decisões cravadas → impacto cross-spec) conforme regra de PR collab; aponta para `spec.md`, `plan.md` e `decision-brief.md` (se houver).
...
- [ ] **4.2** Migração de research: cada arquivo significativo renomeado para incluir a data (`YYYY-MM-DD-nome.md`) e movido para a pasta de domínio correta em `.specify/specs/researchs/<domínio>/`. Adicionar link + resumo em `.specify/specs/research-index.md`. Política completa em `docs/process/spec-foundation.md`.
