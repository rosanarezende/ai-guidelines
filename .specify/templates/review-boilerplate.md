...
- [ ] **R4** — Governança visual consolidada (artefatos OFICIAIS, não anexos): as imagens obrigatórias da spec existem e foram **promovidas para `<spec>/assets/`** — **#1 Visão pretendida**, **#3 Valor entregue** (versão whole-spec) e **#4 Convergência da stack** (esta no Integration PR). Espelha o que `governance-pr-check` já FALHA por-PR (Problema + Valor em Ready; Convergência no Integration). Decida também o **public-facing** (README/CHANGELOG): (a) "não se aplica" + 1 linha; ou (b) atualizado. _R4 não fecha sem o conjunto visual obrigatório promovido._ Evidência: links das imagens em `assets/` + a decisão de README.
