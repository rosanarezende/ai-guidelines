...
5. ~~**Bin físico + yarn scripts bloqueados por TS6133 em `ruleZone.ts`.**~~ **Build destravado em 2026-05-11** via `chore(spec-0021): remove import nao-usado em ruleZone.ts`. `dist/cli/livingDocs.js` agora é materializado por `yarn build`. **Bin físico continua PENDENTE**, mas pelo motivo de design declarado no item 9, não pelo build. Reescrito para apontar para 3.C.9 como bloqueio canônico.
6. **Integração CI depende do bin → depende do gate 9.** `living-docs:check` precisa rodar como job CI dedicado conforme ADR 0003. Bloqueio efetivo é 3.C.9 (agregação por ruleId); CI segue trivial depois que `[3.C.4-prep]` fechar e `[3.C.4]` materializar bin + scripts.
...
9. **Gate de design — agregação por ruleId no Living Docs.** **Sub-bloco `[3.C.4-prep]` aberto** em `tasks.md` (2026-05-11) com decisão fundamentada na auditoria [`./audit-2026-05-11-pre-3c4-living-docs-aggregation.md`](./audit-2026-05-11-pre-3c4-living-docs-aggregation.md). Survey no repo confirma padrão dominante "1 rule → N cenários" (`BR-CLI-A-01` 31×, `BR-CLI-APP-01` 6×, `BR-CLI-POLICY-01` 6×, etc.). 4 opções avaliadas; **Opção (C) escolhida**: schema `v0` evolui in-place ganhando `evidence: SourceLocation[]` plural; fusão determinística de `coverageState` no domain; bypass convergente cross-evidence; cross-file fatal. **Sem bump v0→v1** (artefato nunca foi escrito em produção; ADR 0002 §6 protege evolução de outcomes, não de shape sem consumidor externo). 3 códigos de erro novos: `LIVING_DOCS_INCONSISTENT_DEPRECATION`, `LIVING_DOCS_BYPASS_DIVERGENT`, `LIVING_DOCS_RULE_CROSS_FILE`. `LIVING_DOCS_DUPLICATE_RULE_ID` deixa de ser emitido (sem release pública dependendo). TDD em 4 passos atômicos. **Os 4 passos do 3.C.4 (bin + scripts + CI + close) ficam congelados até `[3.C.4-prep]` fechar.**
10. **Sub-débitos candidatos para `[3.C.aux]` paralelo (auditoria 2026-05-11).** Dois débitos baratos (~1-2h cada) que aproveitam a tooling AST/store em foco: **2.B.4** (RegistryService coverage abaixo de update/remove/load/save + autosave:false nunca testado) e **2.C.5** (`JsonRulesCatalogSource` em 0% coverage). Não bloqueio; pegar em paralelo só se houver folga na sessão de `_prep`. Owner decide.
