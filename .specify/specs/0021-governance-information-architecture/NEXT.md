...
5. ~~**Bin físico + yarn scripts bloqueados por TS6133 em `ruleZone.ts`.**~~ **TOTALMENTE RESOLVIDO em `[3.C.4]` (2026-05-11).** Cadeia de commits: `709c21a` (destrava `yarn build`) → `fff329e` (bin físico) → `2ade096` (yarn scripts + baseline 157 entries versionado). `yarn living-docs:generate` e `yarn living-docs:check` ambos exit 0.
6. ~~**Integração CI depende do bin.**~~ **RESOLVIDO em `[3.C.4]` (2026-05-11).** Step `Validate Living Documentation drift guard` adicionado ao job `ai-guidelines-check` em `.github/workflows/ai-guidelines-ci.yml`. Roda `yarn living-docs:check` após `yarn check`; falha do drift guard ou de qualquer das 5 condições estruturais (AMBIGUOUS, CROSS_FILE, INCONSISTENT_DEPRECATION, BYPASS_DIVERGENT, BYPASS_EXPIRED) quebra CI com mensagem listando o `ruleId` e hint para `yarn living-docs:generate`. Harness Lock garantido.
...
13. **Dívidas abertas pelo polish do review do PR #13 (2026-05-12).** Três itens carregados como débito explícito após o pacote de polish:
    - **`Clock` port adiado.** `TypeScriptRuleExtractorOptions.todayIso` segue como option-com-default (`new Date().toISOString()`); doc do tipo refletida para a realidade. Extrair `Clock` port em `src/app/ports/Clock.ts` quando aparecer o **segundo consumidor de "hoje"** (drift guard de schema, expiração de bypass cross-guard). Porta sem segundo consumidor é abstração prematura.
...
1. ~~**`NodeRecipeStore` adapter não implementado.**~~ **RESOLVIDO em PR13 (2026-05-13).** Adapter criado usando `node:fs` síncrono e testado fim-a-fim contra IO real (`NodeRecipeStore.integration.test.ts`). O uso de chamadas síncronas para templates (poucos KBs) lidos sob demanda tem impacto negligenciável, ficando a conversão para promise como débito v1 se justificado por performance.
2. **Recipes completas e conversão da CLI.** A PoC (`tasks-evidence-driven` e seus partials) foi criada para provar o motor, mas as receitas completas para `spec`, `plan`, `decision-brief`, etc., bem como a virada do comando CLI para parar de copiar de `.specify/templates/` e usar as recipes, ficam para o PR4 (durante o `AdoptWorkspace`).
3. ~~**`ARCHITECTURE.md` §B.1 e §G não atualizados.**~~ **RESOLVIDO.** Itens adicionados ao mapa de bounded contexts e glossário.
...
2. **`governance-foundation.md` ainda com nome legado** (`/.core/process/governance-foundation.md`). Catálogo aponta para o path com nota "(renomeado em 4.B.1)". Renomeação/refatoração ocorre em 4.B.1; dissonância cognitiva leve mas não bloqueia o catálogo.
