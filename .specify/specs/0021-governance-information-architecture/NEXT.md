...
4. **Bypass directive declarada em ADR 0003 não está implementada como parser.** O schema do living-docs **aceita** o bloco `bypass { until, ref, reason }` em entries deprecated, mas **não há ainda** um parser que reconheça o comentário `// living-docs:allow-drift ...` em código TypeScript e produza essa entrada. Esse parser nasce com o AST extractor em PR3.B. Por ora: se um teste real precisar marcar uma regra como `deprecated`, o autor passa o bloco programaticamente; o reconhecimento sintático em comentário entra em 3.B.
