...
4. **Bypass directive declarada em ADR 0003 não está implementada como parser.** O schema do living-docs **aceita** o bloco `bypass { until, ref, reason }` em entries deprecated, mas **não há ainda** um parser que reconheça o comentário `// living-docs:allow-drift ...` em código TypeScript e produza essa entrada. Esse parser nasce com o drift guard em PR3.C. Por ora: se um teste real precisar marcar uma regra como `deprecated`, o autor passa o bloco programaticamente.
...
9. **Gate de design aberto antes do 3.C.4 — `LIVING_DOCS_DUPLICATE_RULE_ID` em árvore real.** Tentativa de rodar `runGenerate` contra o repo (após destravar `yarn build` em 2026-05-11 via `chore(spec-0021): remove import nao-usado em ruleZone.ts`) falha porque o extractor emite **1 entry por `it`**, e o domínio rejeita ruleIds duplicados. Survey no repo: `BR-CLI-A-01` aparece 31×, `BR-CLI-APP-01` 6×, `BR-CLI-POLICY-01` 6×, `BR-CLI-APP-02` 4×, `BR-CLI-REGISTRY-01` 2× — o padrão BDD "1 rule → N cenários GIVEN/WHEN/THEN" é dominante e legítimo. Decisão pendente: o modelo "1 entry por test location" é incorreto; precisa virar "1 entry por ruleId" agregando locais de evidência + estado de coverage consolidado (covered ⊕ pending ⊕ deprecated). Bloqueia 3.C.4 (bin físico só faz sentido depois). Caminhos a avaliar: (a) `canonicalizeArtifact` deduplica por ruleId mantendo lista de `evidence: SourceLocation[]`; (b) extractor pré-agrega antes de chamar `assertValidArtifact`; (c) schema v0 ganha `evidence` plural e migra para v1 — preferível esperar para evitar churn. Recomendação: tratar como sub-bloco próprio (3.C.4-prep) em sessão dedicada, com TDD começando por teste vermelho no `LivingDocsArtifact` ou `GenerateLivingDocs`. **Os 4 passos do 3.C.4 (bin + scripts + CI + close) ficam congelados até esse gate fechar.**
