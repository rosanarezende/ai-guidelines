...
- **spec 0020** — npm-publication
  ...
  **Lição operacional cravada (2026-05-08):** sequência de publish foi reordenada **antes do ato irreversível** após o owner detectar erro de sequência (a sequência original colocava publish na branch antes do merge, o que deixaria a tag `v1.0.0` órfã da história principal porque o repo usa **squash merge** — squash gera commit-novo em `main` que não existe na branch). A discussão evoluiu para uma decisão de framework: o gap entre "PR auto-suficiente" e o ato pós-merge de release virou padrão **Mini-PR de Release Sync (Fase 5)**, e ficou cravada em `.core/process/spec-foundation.md` a regra de bloqueio de nova spec enquanto Release Sync estiver pendente. Sequência canônica documentada: **merge → checkout `main` → publish → tag → Mini-PR de Release Sync**. Vale para releases futuras (1.0.1, 1.1.0, e specs subsequentes que publiquem em registry externo).

  **Releases publicados:**
  - **`ai-guidelines@1.0.0`** — publicado em 2026-05-08 a partir do commit `9ef875a` em `main` (squash merge do PR #6); tag `v1.0.0` ancorada nesse commit. Registry: <https://www.npmjs.com/package/ai-guidelines/v/1.0.0>. ⚠️ **Release com bug crítico:** `cli/ai-guidelines-cli.mjs` foi publicado sem shebang `#!/usr/bin/env node`, fazendo o shim do npm em Windows tentar executar o `.mjs` diretamente (sistema usava associação de extensão / editor) e gerar exit 0 silencioso sem rodar a CLI. Em Unix, cenário análogo. Bug detectado pela mantenedora ao testar o consumer flow logo após o publish; smoke do repo passava porque invocava o entrypoint via `node <path>` direto, não via shim do npm. Corrigido em `1.0.1`.
  - **`ai-guidelines@1.0.1`** — publicado em 2026-05-08 a partir do commit `2bd4af3` em `main` (squash merge do PR #7); tag `v1.0.1` ancorada nesse commit. Registry: <https://www.npmjs.com/package/ai-guidelines/v/1.0.1>. ✅ **Hotfix funcional**: shebang adicionado, smoke `tests/smoke/bin-shim.test.mjs` cobrindo invocação real via shim do npm (estático + comportamental), README reescrito como landing pública (-65%), `CONTRIBUTING.md` ampliado com setup local, `docs/cli/ai-guidelines-cli.md` ganhou seção 0 com política de update. Validado pós-publish via `npx ai-guidelines@1.0.1` em sandbox limpo (artefatos completos criados em target vazio).

  **Lição operacional adicional cravada (2026-05-08, durante hotfix 1.0.1):** smoke deve cobrir o **caminho real do consumidor** (invocação via `node_modules/.bin/<name>` ou `npx`), não apenas o entrypoint isolado via `node <bin-path>`. Invocar diretamente simula o resultado que o shim deveria entregar; não simula o shim em si. Regra refletida em `tests/smoke/bin-shim.test.mjs`.
