...
- **Spec 0020 — `npm-publication`** (branch: `0020-npm-publication`)
  ...
  - **Escopo resumido:** ajustar `package.json` para publish (remover `private`, adicionar `license`/`repository`/`homepage`/`bugs`/`keywords`); ADR de naming registrando `ai-guidelines` não-scoped como decisão final; smoke tests de `npx ai-guidelines init` em diretório vazio; ativar `pr-curator` como GH Action real; CI multi-SO (Windows/Linux/macOS) sobre o tarball; doc consumer-facing no `README` (substituir orientação `yarn guidelines` por `npx ai-guidelines`); publish 1.0.0 (versionamento real, sem placeholder — `package.json` está em 1.4.0 interno mas nunca foi publicado, então 1.0.0 inicia série pública).
...
- **npm-publication** (spec **0020** — Publicação npm + automação cross-repo) — **selecionada para execução**
  ...
  - **Escopo:** ajustar `package.json` (remover `"private": true`; adicionar `license`, `repository`, `homepage`, `bugs`, `keywords`, opcional `engines`); registrar ADR de naming (`ai-guidelines` não-scoped como package principal; `@ai-guidelines/<addon>` reservado para extensões futuras); smoke tests de `npx ai-guidelines init` em diretório vazio e em projeto existente (incluindo verificação do tarball via `npm pack --dry-run`); ativar `pr-curator` como GH Action real em repositório da mantenedora; CI multi-SO (Windows/Linux/macOS) rodando smoke tests sobre o tarball; documentação consumer-facing no `README` (substituir orientação `yarn guidelines` interna por `npx ai-guidelines` para usuários externos, mantendo seção de contribuidor distinta); publish 1.0.0 (1.4.0 interno nunca saiu — 1.0.0 inicia série pública).
  - **Critérios de aceite:** `npx ai-guidelines init` funciona em diretório vazio (smoke test verde); `update` aplica patches sob `managed-block` em consumidor recém-init; CI verde nos 3 SOs sobre o tarball; ADR de naming registrado; `pr-curator` Action ativa; pacote 1.0.0 publicado e instalável; README com guia de instalação consumer-facing.
