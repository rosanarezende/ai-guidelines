...
- **governance-information-architecture** (spec **0021** — Auditoria + classificação canônica de informação essencial do framework) — **próxima na fila** _(Spec 0020 mergeada em 2026-05-08; ver `historico.md`.)_
  ...
  - **Ampliação de escopo aprovada como insumo (2026-05-08):** além da classificação documental, a 0021 deve decidir o **modelo canônico de estado** do framework para backlog/histórico/artefatos de origem. Direção preferencial atual: **repo-first híbrido** — registro estruturado versionado como fonte primária, `backlog.md`/`historico.md` como visões derivadas úteis para IA, e projeções futuras (ex.: SQLite/dashboard) fora da fonte de verdade inicial.
  ...
  - **Fases propostas para a própria 0021:** **Fase 1** — definir taxonomia de artefatos (`prd`, `incident`, `friction`, `note`, `spec`, `delivery`, `adr`), relações e campos mínimos; **Fase 2** — introduzir o registro estruturado canônico no repositório; **Fase 3** — gerar `backlog.md`, `historico.md` e possivelmente `status.json` como visões derivadas. **Fases 4 e 5** (SQLite local-first, dashboard/web/backend) devem ser apenas mapeadas e deixadas como evolução posterior.
  ...
  - **Insumo de arquitetura de estado (2026-05-08):** a necessidade de referenciar PRDs, incidentes, friction reports e entregas `no-spec` sem tirá-los do fluxo de valor fortaleceu a proposta de um modelo híbrido: **estado estruturado versionado no repo + Markdown derivado + projeções futuras**. Isso permite que backlog/histórico deixem de ser apenas narrativa e passem a funcionar como índice derivado de entidades relacionáveis, sem trocar Git por banco como verdade primária. Research de apoio: `.specify/specs/researchs/architecture/2026-05-08-repo-first-structured-registry.md`.
