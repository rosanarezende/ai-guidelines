...
- **Spec 0020 — `npm-publication`** (branch: `0020-npm-publication`)
  - **Status:** preparação (branch criada 2026-05-07, backlog reorganizado, naming decision resolvida; `spec.md`/`plan.md`/`tasks.md` pendentes).
  - **Tipo:** `deterministic` (publicação de pacote + tooling, sem decisão arquitetural pendente).
  - **Escopo resumido:** ajustar `package.json` para publish (remover `private`, adicionar `license`/`repository`/`homepage`/`bugs`/`keywords`); ADR de naming registrando `ai-guidelines` não-scoped como decisão final; smoke tests de `npx ai-guidelines init` em diretório vazio; CI multi-SO (Windows/Linux/macOS) sobre o tarball; doc consumer-facing no `README` (substituir orientação `yarn guidelines` por `npx ai-guidelines`); publish 1.0.0 (versionamento real, sem placeholder — `package.json` está em 1.4.0 interno mas nunca foi publicado, então 1.0.0 inicia série pública).
  - **Recorte de escopo 2026-05-08:** ativação do `pr-curator` como GH Action ativa **extraída** desta spec — auditoria mostrou que o comando não existe na CLI; transferido para spec candidata `pr-curator-action` em **Next**.
  - **Detalhes completos:** ver entrada em **Now** abaixo (mantida como referência durante execução; será movida para `historico.md` no merge).
  - **Reordenação 2026-05-07:** esta spec foi promovida sobre `governance-information-architecture` (que era a 0020 candidata e agora é a **0021**). Justificativa: package.json já está 90% pronto, antecipação destrava narrativa de portfólio (pacote instalável + dashboard medindo uso real) e libera a info-architecture para entrar com mais espaço de respiração na próxima rodada.

> **Spec 0019** (`bootstrap-consumidor-e-runtime`) concluída e mergeada (PR #5, commit `35af73a`): ver entrada em [`historico.md`](./historico.md).
...
- **npm-publication** (spec **0020** — Publicação npm) — **selecionada para execução**
  - **Fonte da promoção:** auditoria do `package.json` em 2026-05-07 revelou que o pacote está ~90% pronto para publicação (nome `ai-guidelines` já configurado, `bin`/`files`/`dependencies` corretos). Único bloqueador estrutural era o flag `"private": true`. Combinada com a decisão de naming resolvida (`ai-guidelines` não-scoped) e a urgência de portfólio, justifica antecipar 0006 sobre `governance-information-architecture` (que era candidata a 0020 e agora é **Spec 0021**). Promovida de Next em 2026-05-07.
  - **Escopo:** ajustar `package.json` (remover `"private": true`; adicionar `license`, `repository`, `homepage`, `bugs`, `keywords`, opcional `engines`); registrar ADR de naming (`ai-guidelines` não-scoped como package principal; `@ai-guidelines/<addon>` reservado para extensões futuras); smoke tests de `npx ai-guidelines init` em diretório vazio e em projeto existente (incluindo verificação do tarball via `npm pack --dry-run`); CI multi-SO (Windows/Linux/macOS) rodando smoke tests sobre o tarball; documentação consumer-facing no `README` (substituir orientação `yarn guidelines` interna por `npx ai-guidelines` para usuários externos, mantendo seção de contribuidor distinta); publish 1.0.0 (1.4.0 interno nunca saiu — 1.0.0 inicia série pública).
  - **Critérios de aceite:** `npx ai-guidelines init` funciona em diretório vazio (smoke test verde); `update` aplica patches sob `managed-block` em consumidor recém-init; CI verde nos 3 SOs sobre o tarball; ADR de naming registrado; pacote 1.0.0 publicado e instalável; README com guia de instalação consumer-facing.
  - **Recorte 2026-05-08:** ativação do `pr-curator` como GH Action **removida do escopo** desta spec; transferida para a candidata `pr-curator-action` em Next (auditoria revelou que a feature CLI não existe; implementar do zero extrapola "publicação npm").
  - **Pré-requisitos satisfeitos:** Spec 0005 concluída (✓); Spec 0019 mergeada (✓ — entrega contrato de update via `managed-block` + comando `update`); naming decision **resolvida em 2026-05-07** (`ai-guidelines` não-scoped — ver bloqueador #1).
  - **Independência da Spec 0021:** 0020 não toca em `docs/` profundamente nem em `adrs/` (apenas adiciona um ADR), nem em `.core/rules/` — paths que a 0021 reorganiza. Sequência 0020 → 0021 evita churn no `README` durante publicação e garante que o pacote já estará no NPM quando a 0021 entrega o sub-bloco antecipatório de telemetria (lar para o dashboard).
  - **Riscos antecipados:** GitHub tokens cross-repo exigem PAT fino ou GitHub App; Action que abre PRs em outro repo pode virar ruído sem gatilho correto (label `growth-relevant`); NPM org paga vs GitHub Packages — decisão a registrar em ADR (recomendação: registry público padrão, gratuito; org paga só se justificar acesso privado futuro); smoke test pode revelar bugs ocultos no init (mitigar publicando como `1.0.0-rc.1` antes da 1.0.0 final se houver dúvida); janela de unpublish do npm é **72 horas** — usar antes desse prazo se algo crítico for descoberto.
  - **Cross-ref:** estratégia de update já resolvida pela 0019 (sub-bloco C); 0020 só publica sob esse contrato. Dashboard (Now #4) consome eventos de instalação pós-publicação para medir adoção real.

- **governance-information-architecture** (spec **0021** — Auditoria + classificação canônica de informação essencial do framework) — **próxima na fila após Spec 0020**
...
- **process-automations** (Automatização de ciclo de vida de Gaps via CLI)
  ...
  - **Escopo potencial:** criar workflow/comando no CLI que facilite a alimentação de `NEXT.md` e `backlog.md` a partir de insights capturados no chat; comando `ai-guidelines spec init <slug>` (scaffold de fundação de spec, incorporando o item oportunista correspondente) sobre o lar canônico definido pela 0020.
