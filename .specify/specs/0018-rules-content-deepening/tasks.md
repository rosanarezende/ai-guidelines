...
- [ ] **5.B3.1** Definir schema YAML formal de uma regra: campos `id`, `scope` (`universal` | `adapter` | `opt-in`), `adapter` (opcional), `opt_in_feature` (opcional), `category` (`correctness` | `security` | `maintainability` | `process` | `editorial`), `evidence_strength` (`strong` | `medium` | `emerging` | `declared_heuristic`), `sources` (lista, **obrigatório apenas para categorias-âncora `correctness`/`security` com `evidence_strength: strong|medium`**), `applicable_languages`, `tags`. Documentar em `decision-brief-boilerplate.md` (4.A4) e em `CLAUDE.md` raiz. **Garantir que `category: process|editorial` aceita `declared_heuristic` sem `sources` (cobre Convenções do Owner + diretivas core).** **Padrão YAML cravado em 2026-05-03 (alinha com B.3.1.5):** **frontmatter `---` real** (Opção A — padrão de mercado). `rules-parser.mjs` (B.3.2) deve procurar YAML assim: cada regra inicia com bloco `---\n<yaml>\n---` imediatamente seguido por um heading `## [ID] Title` na próxima linha não-vazia. Em arquivos multi-regra (como `agents-core.md`, futuro `global-rules.md` migrado), múltiplos blocos frontmatter coexistem — o parser isola cada bloco pelo padrão `---\n...\n---\n## ...`.
- [ ] **5.B3.1.5** **AGENTS-core como catálogo revisável (ledger) usando schema YAML.** Origem: gap identificado em 2026-05-03 — `.core/templates/AGENTS-core.md.tmpl` é o conteúdo mais caro em tokens do framework, sempre injetado, e nunca passou por revisão crítica.
  - [/] **5.B3.1.5.1** Criar `.core/rules/agents-core.md` com 13 diretivas (CORE-01 a CORE-13) extraídas do template, no schema YAML de B.3.1 + corpo bilíngue. **Não-injetado** enquanto B.3.5 não rodar (catálogo paralelo). Tags: `[core, agents, always_injected]`. _Pendente de commit (Política A — task só vira `[x]` quando há commit incremental correspondente). Conteúdo escrito em 2026-05-03; revisão editorial em 2026-05-03 (frontmatter `---`, cross-refs corrigidas, Instruction (en) encurtada em CORE-02/CORE-08/CORE-11)._
  - [ ] **5.B3.1.5.2** Em B.3.4 (builder): filtrar regras com `tags: core` e gerar `.core/rules/_meta/agents-core-ledger.md` automaticamente, com tabela `id | title | category | evidence_strength | sources | chars | lines` ordenada por `id` (estabilidade).
...
- [ ] **5.B3.4** Implementar `cli/governance/monolith/rules-builder.mjs`: serializa o catálogo completo em `rules.json` (build artifact). Estrutura: `{ rules: [...], by_id: {...}, by_scope: {...}, generated_at, schema_version }`. **Adicionalmente:** filtrar regras com `tags: core` e gerar `.core/rules/_meta/agents-core-ledger.md` (tabela ordenada por `id`, ver 5.B3.1.5.2). Tests co-located.
