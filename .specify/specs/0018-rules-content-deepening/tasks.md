...
- [x] **5.B3.4** Implementar `cli/governance/monolith/rules-builder.mjs`: serializa o catálogo completo em `rules.json` (build artifact). Estrutura: `{ rules: [...], by_scope: {...}, generated_at, schema_version }`. **Adicionalmente:** filtrar regras com `tags: core` e gerar `.core/rules/_meta/agents-core-ledger.md` (tabela ordenada por `id`, ver 5.B3.1.5.2). Tests co-located. **Concluído 2026-05-04:** ~300 linhas, builder caseiro sem dependências externas. Suporta: 3 índices (rules[], by_scope, by_feature), validação cross-índice, geração de ledger markdown, fail-fast com acumulação de erros. Testes 31/31 validam completamente (discovery, indexing, validação, core ledger, metadata, edge cases). Commit: `feat(spec-0018): 5.B3.4 rules-builder.mjs + 31/31 BDD tests verdes` (2026-05-04).
