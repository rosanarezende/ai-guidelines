...
- **2026-05-03 — Reabertura controlada de `[DEC-0018-B08]` (introdução do estado `mover (convenção)`).**
  - **Mudança:** durante 5.B2.1/5.B2.2 (inventário das 24 regras candidatas e atribuição de source canônica), owner identificou que a aplicação estrita do critério **E (source obrigatória)** levaria à perda de regras pain-driven que economizam tokens, evitam retrabalho ou previnem comportamentos perigosos da IA — sem que tenham CWE/CERT/Sonar/OWASP direto. Solução: introduzir um **quarto estado de decisão** (`mover (convenção)`) que preserva o conteúdo numa categoria editorialmente honesta, separada do núcleo evidence-driven. Aplicado a 3 regras nominalmente identificadas: **PE-01** (PT-BR), **EF-04** (Redução de Ruído / ignore-files) e **WF-03** (PR description colaborativo 3 etapas). Adicionalmente: cravado débito B.4 para a regra **nova** "IA gera apenas texto do commit; humano executa cadeia manual" — não está no inventário b9efb83 (não existe em `.core/rules/*`), mas é prática operacional do `AGENTS.md` raiz que merece distribuição a consumidores.
  - **Por quê:** purga radical pura aplicada a regras pain-driven enfraqueceria o framework por motivos errados (perda de aprendizado prático), enquanto manter sem cerimônia enfraqueceria o núcleo evidence-driven (perda de credibilidade). A categoria "Convenção do Owner" preserva a **honestidade editorial**: regras úteis vivem em seção/arquivo que declara explicitamente "sem evidência empírica direta", separadas das regras com source canônica.
  - **Implicação no decision-brief:** `[DEC-0018-B08]` Sub-eixo 2 cravou **E estrito** em 2026-04-30 — esta entrada **expande** o critério (não reverte) com um regime adicional para regras com `valor declarado pelo owner` mas sem source. Brief permanece `Resolved`; mudança vive aqui (sem reabrir gate humano formal). Status da decisão: política expandida pós-gate.
  - **Implicação em B.4:** decisão de destino físico das convenções (opt-in module dedicado vs. seção segregada em `global-rules.md`) fica para B.4 quando o schema bilíngue + IDs canônicos forem aplicados. EF-04 tem rota paralela: vira automação na candidata "Scaffolding Inteligente de Provedores" (já em § B.7 → `NEXT.md`).
  - **Implicação em B.4:** decisão de destino físico das convenções (opt-in module dedicado vs. seção segregada em `global-rules.md`) fica para B.4 quando o schema bilíngue + IDs canônicos forem aplicados. EF-04 tem rota paralela: vira automação na candidata "Scaffolding Inteligente de Provedores" (já em § B.7 → `NEXT.md`).
  - **Implicação em B.2:** tabela de reconciliação em `research/2026-04-30-b9efb83-reconciliation.md` ganha § 1.1 ("Política adicional pós-gate") + 4ª coluna de decisão; síntese § 3.1/3.2/3.3 atualizada (5 mantidas / 3 revisar / **3 movidas para Convenções** / 13 revertidas + 1 sub-N+1). 5.B2.5 (aplicação física das reversões) preserva PE-01/EF-04/WF-03 numa nova seção "Convenções do Owner" ao final de `global-rules.md` com disclaimer explícito.

- **2026-05-03 — Decisão cravada: parser caseiro vs lib externa YAML (B.3.2).**
  - **Mudança:** antes de iniciar B.3.2 (rules-parser.mjs), realizamos análise sistemática (tabela Eixo × Vantagens/Desvantagens) comparando (a) parser caseiro, (b) lib `yaml` ou `js-yaml`. **Decisão cravada: parser caseiro.**
  - **Por quê:** subset YAML necessário é trivialmente restrito (7 campos enumerados, tipos só `string` e `array of string`, sem objetos aninhados, multi-line strings, âncoras/aliases). Vantagem clássica de lib externa (cobertura edge-cases YAML) não se aplica. `package.json` do framework é dep-free hoje (ideologia do projeto); adicionar lib externa muta lockfile, contamina install dos consumidores via `init`/`adopt`, cria semver risk. Cobertura BDD/TDD completa do subset (~36 specs `[BR-PARSER-NN]`) substitui as garantias da lib.
  - **Implicação em B.3:** parser caseiro ~80–120 linhas + ~400–500 linhas de testes (BDD verboso); removes 1 decisor crítico em B.3.2 (executa com escopo de parser cravado). Full coverage e fail-fast alinha com CLAUDE.md raiz (tipagem estrita, estrutura, mutações de parser suportam test-driven).
  - **Implicação em `package.json`:** zero alteração na lista de dependências (permanece dep-free).

- **2026-05-03 — Débito cravado: nova regra "IA gera apenas texto do commit; humano executa cadeia manual" (candidata B.4).**
  - **Mudança:** durante 5.B2 (inventário b9efb83), identificou-se que a prática operacional existente no `AGENTS.md` raiz ("IA não executa `git push` isolado; toda submissão segue `yarn format ; yarn check ; git add . ; git commit -m ...`") **não existe como regra no catálogo** (inventário incluiu apenas 24 existentes em `.core/rules/*` + `opt-in/quality-gates.md`). Essa prática é **pain-driven, operacional e transferível** aos consumidores. Categorizada como `process|editorial` (sem source canônica, `declared_heuristic`). Será criada em B.4 como nova regra com ID `[????]` (número alocado durante B.4).
  - **Por quê:** ao documentar o framework, descobrir que a política mais operacional (aquela que **protege** o repo de commits malformados) não vive no catálogo é uma lacuna. Adicionar em B.4 fecha a lacuna sem refazer B.2 (que foi fechado com gate humano).
  - **Implicação em B.4:** candidata aguarda schema bilíngue + builders. Seção em `global-rules.md` ("Workflow com IA") já menciona indiretamente; nova regra explicitará o contrato.
  - **Implicação em `NEXT.md`:** zero — é parte de B.4 (Stage 2 normal), não débito.
