...
> **Modelo de 3 boundaries (Spec 0023 — `[DEC-0023-M01]`):** as fases de Review e Encerramento acima estão migrando para artefatos dedicados — **`review.md`** (integration readiness; gates R1–R7, lido pelo runtime) e **`release-log.md`** (log de operações pós-merge). O `tasks.md` torna-se **execution-only** (fecha 100% `[x]` ao fim da execução); os boilerplates já refletem isso. A reconciliação completa desta seção e do "Princípio de PR auto-suficiente" abaixo com o modelo (incl. o papel pós-merge do `release-log.md`) é débito de fechamento da 0023 — ver `.governance/specs/0023-workflow-runtime/`.
