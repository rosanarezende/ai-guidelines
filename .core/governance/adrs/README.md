...
- **2026-05-22 (Spec 0023 / pós-PR4 validação empírica):** ADR 0022 (Handoff situado em estado precede distribuição pré-carregada de regras) criada em status Proposta após observação direta: sessão de IA iniciada com handoff redigido manualmente alcançou aderência ao processo qualitativamente superior à de sessões iniciadas apenas com `AGENTS.md` + arquivos por canal. Princípio canônico: bootstrap de sessão IA é handoff just-in-time situado em estado, não distribuição pré-carregada. Reposiciona `AGENTS.md`/`.cursorrules`/`CLAUDE.md`/`.github/copilot-instructions.md` como stubs orientativos de invocação; move apresentação contextual para o handoff. Complementar a ADR 0021: 0021 fortifica a borda; 0022 simplifica a entrada. Validação plena depende de wizard CLI da Spec 0023 + materialização do comando handoff (Spec 0024 candidata).
