...
| 0022 | Handoff situado em estado precede distribuição pré-carregada de regras               | Bootstrap de sessão IA é handoff just-in-time; AGENTS.md vira stub; geração determinística com slots híbridos opcionais                      | **Proposta** (Spec 0023 / pós-PR4 validação empírica) |
| 0023 | Meta-artefatos de governança são SSOT YAML com derivações determinísticas build-time | YAML SSOT editável humano + JSON para scripts + HTML para stakeholders; build-time, sem LLM; cláusula anti-paper exige materialização rápida | **Proposta** (Spec 0023 / PR5 S3 sanitização)         |
| 0024 | Draft, Ready e Mergeable são estados distintos em PRs governance-first               | Modelo de 3 estados: Draft (WIP) ≠ Ready (review) ≠ Mergeable (stack inteira + autorização explícita); operacionaliza ADR 0020 no template   | Aceita (Spec 0023 / DEC-0023-J01)                     |
...
- **2026-05-22 (Spec 0023 / pós-PR4 validação empírica):** ADR 0022 (Handoff situado em estado precede distribuição pré-carregada de regras) criada em status Proposta após observação direta: sessão de IA iniciada com handoff redigido manualmente alcançou aderência ao processo qualitativamente superior à de sessões iniciadas apenas com `AGENTS.md` + arquivos por canal. Princípio canônico: bootstrap de sessão IA é handoff just-in-time situado em estado, não distribuição pré-carregada. Reposiciona `AGENTS.md`/`.cursorrules`/`CLAUDE.md`/`.github/copilot-instructions.md` como stubs orientativos de invocação; move apresentação contextual para o handoff. Complementar a ADR 0021: 0021 fortifica a borda; 0022 simplifica a entrada. Validação plena depende de wizard CLI da Spec 0023 + materialização do comando handoff (candidata `handoff-as-first-class` no backlog, per ADR 0017).
- **2026-05-22 (Spec 0023 / PR5 S3 sanitização):** ADR 0023 (Meta-artefatos de governança são SSOT YAML com derivações determinísticas build-time) criada em status Proposta após dois sinais convergentes: análise do formato do backlog (markdown vs YAML+JSON+HTML) e reconhecimento do débito de dashboard de governança que nunca saiu do papel desde a época do `living-docs.yml`. Princípio canônico: YAML SSOT editável humano + JSON para scripts + HTML estático para stakeholders, derivações build-time determinísticas (sem LLM no runtime, ADR 0018 preservado). Padrão idiomático ao projeto — já praticado isoladamente em `agents-core.md` → AGENTS.md/ledger/rules.json + `state.yml` → active-specs.yml + `living-docs.yml`. Cláusula anti-paper explícita: ADR só vale se houver materialização rápida (candidata `governance-dashboard-and-visual-artifacts` como próxima spec após 0023). NÃO aplica a markdown narrativo (specs/ADRs/decision-briefs continuam markdown puro).
- **2026-05-23/24 (Spec 0023 / DEC-0023-J01, Frente #3 do hardening do PR #25):** ADR 0024 (Draft, Ready e Mergeable são estados distintos em PRs governance-first) criada em status Aceita após confusão empírica observada em múltiplas sessões de review da stack `#18→#19→#22→#23→#24→#25`: Codex, Claude e Antigravity trataram PRs convertidos para `Ready` como autorizados para merge mesmo a stack tendo PRs upstream/downstream pendentes. Princípio canônico: 3 estados distintos — Draft (WIP, não solicita review) ≠ Ready (operacionalmente concluído, aguarda review humano) ≠ Mergeable (stack inteira Ready + autorização explícita do owner para merge atômico ponta-a-ponta per ADR 0020). Articula o modelo que CORE-09/10/16 tangenciavam sem cravar. Materializado em redesign do `.github/pull_request_template.md` (Commit B do par governance+implementation cravado em DEC-0023-J01). Trata de **WHAT** os estados significam — questão complementar e distinta de `[DEC-0023-F05]` (Deferred com critério estrutural), que trata de **WHERE** a SSOT de CORE-09/10 vive.
