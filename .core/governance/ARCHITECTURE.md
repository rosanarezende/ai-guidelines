...
**Estado pós-merge da 0021:** o framework opera como sistema governance-driven com `.governance/` declarado como root canônico do consumidor, `.ai-guidelines/` mantido como bridge legada explícita até o nó `dualroot-collapse` da Spec 0024. A engine de templates está ativada para a recipe `tasks-evidence-driven`; demais templates continuam vindo do mirror legado `.specify/templates/` até o mesmo colapso. Living Documentation com drift guard fatal protege as 235+ entries em `.governance/living-docs.yml`. ADRs consolidadas em `.core/governance/adrs/` (8 ativas + repositioning governance-first via ADR 0018). Trilha de fechamento disciplinada em `.specify/specs/0021-governance-information-architecture/closure-review.md`.
